...
Агентский договор используется для сценария: агент продает емкость перевозчика, оформляет MAWB/HAWB и
отчитывается по комиссиям. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется по
...
booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала, переписка с перевозчиком/GHA. AWB/MAWB/HAWB,
airport, flight, terminal, customs и dangerous goods указываются только как условия или ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «агентирование грузовой емкости» и есть операционная связка
...
- **Агентирование грузовой емкости** — профиль документа, который определяет обязательные приложения и evidence.
- **Aviation evidence по сделке** — booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала, переписка с
  перевозчиком/GHA; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — превышение полномочий агента или неподтвержденная комиссия; условие выносится в
  приложение, SLA или протокол разногласий.
- **Эскалационное событие** — спор по комиссии, отказ в приемке груза или расхождение отчета агента с AWB; после
...
- Владелец процесса проверяет профиль «агентирование грузовой емкости», стороны, authority matrix, коммерческие
  условия и evidence: booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала, переписка с перевозчиком/GHA.
...
- Эскалация обязательна при событии: спор по комиссии, отказ в приемке груза или расхождение отчета агента с
  AWB. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «агентский договор, комиссия, посредник» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Агентский договор» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала, переписка с
  перевозчиком/GHA.
...
- Ограничение для ответа: превышение полномочий агента или неподтвержденная комиссия. Если источник не
  подтверждает его, RAG-ответ указывает пробел и не подменяет его общей договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Агентский договор» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «агентирование грузовой емкости», liability cap, waiver и нестандартные приложения |
| Commercial air cargo lead | подтверждает operational facts: booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала, переписка с перевозчиком/GHA |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Агентский договор» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «агентирование грузовой емкости»: booking, отчет
  агента, MAWB/HAWB, rate sheet, акт терминала, переписка с перевозчиком/GHA.
- Ключевой риск сценария: превышение полномочий агента или неподтвержденная комиссия.
...
- ответ решает именно сценарий «агентирование грузовой емкости» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала,
  переписка с перевозчиком/GHA
- учтен профильный риск: превышение полномочий агента или неподтвержденная комиссия
...
- [ ] запрос сопоставлен с документом «Агентский договор» и профилем «агентирование грузовой емкости»
- [ ] собраны профильные evidence: booking, отчет агента, MAWB/HAWB, rate sheet, акт терминала, переписка с
  перевозчиком/GHA
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: превышение полномочий агента или неподтвержденная комиссия
...
Внеплановый пересмотр нужен после события: спор по комиссии, отказ в приемке груза или расхождение отчета агента
с AWB; также после изменения carrier/GHA rules, airport security требований, customs/export control режима,
страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «агентирование грузовой емкости» и
...
> Применять «Агентский договор» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
