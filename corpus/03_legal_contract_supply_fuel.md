...
Договор поставки топлива используется для сценария: поставляются ГСМ для наземной техники cargo terminal и
аварийного энергоснабжения. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется по
...
fuel specification, batch certificate, delivery ticket, storage rules, consumption log, safety sheet, acceptance
act. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «поставка топлива/ГСМ для GSE» и есть операционная связка с
...
- **Поставка топлива/гсм для gse** — профиль документа, который определяет обязательные приложения и evidence.
- **Aviation evidence по сделке** — fuel specification, batch certificate, delivery ticket, storage rules,
  consumption log, safety sheet, acceptance act; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — ГСМ смешивается с aviation fuel topic или поставляется без quality certificate
  и safety controls; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — contaminated batch, delivery delay, spill incident или disputed volume; после него
...
- Владелец процесса проверяет профиль «поставка топлива/ГСМ для GSE», стороны, authority matrix, коммерческие
  условия и evidence: fuel specification, batch certificate, delivery ticket, storage rules, consumption log,
  safety sheet, acceptance act.
...
- Эскалация обязательна при событии: contaminated batch, delivery delay, spill incident или disputed volume.
...
- Предметная проверка: признаки «топливо, ГСМ, поставка, ценообразование» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор поставки» по теме «Договор поставки топлива» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: fuel specification, batch certificate, delivery ticket, storage rules, consumption log,
  safety sheet, acceptance act.
...
- Ограничение для ответа: ГСМ смешивается с aviation fuel topic или поставляется без quality certificate и
  safety controls. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор поставки топлива» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «поставка топлива/ГСМ для GSE», liability cap, waiver и нестандартные приложения |
| GSE fuel owner | подтверждает operational facts: fuel specification, batch certificate, delivery ticket, storage rules, consumption log, safety sheet, acceptance act |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор поставки топлива» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «поставка топлива/ГСМ для GSE»: fuel
  specification, batch certificate, delivery ticket, storage rules, consumption log, safety sheet, acceptance
  act.
- Ключевой риск сценария: ГСМ смешивается с aviation fuel topic или поставляется без quality certificate и
  safety controls.
...
- ответ решает именно сценарий «поставка топлива/ГСМ для GSE» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: fuel specification, batch certificate, delivery ticket, storage
  rules, consumption log, safety sheet, acceptance act
- учтен профильный риск: ГСМ смешивается с aviation fuel topic или поставляется без quality certificate и safety
  controls
...
- [ ] запрос сопоставлен с документом «Договор поставки топлива» и профилем «поставка топлива/ГСМ для GSE»
- [ ] собраны профильные evidence: fuel specification, batch certificate, delivery ticket, storage rules,
  consumption log, safety sheet, acceptance act
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: ГСМ смешивается с aviation fuel topic или поставляется без quality certificate и
  safety controls
...
Внеплановый пересмотр нужен после события: contaminated batch, delivery delay, spill incident или disputed
volume; также после изменения carrier/GHA rules, airport security требований, customs/export control режима,
страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «поставка топлива/ГСМ для GSE» и обновляет
...
> Применять «Договор поставки топлива» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
