...
title: Претензия по аренде ULD, температурного контейнера или терминальной зоны
...
# Претензия по аренде ULD, температурного контейнера или терминальной зоны

## Назначение

Претензия по аренде ULD, температурного контейнера или терминальной зоны используется для сценария: спор связан
с арендой ULD, temperature container, staging area или storage cell для авиагруза. Это претензия контрагенту для
конкретного air cargo dispute, а не общая форма без shipment evidence.

Позиция строится на доказательствах: lease order, equipment ID, acceptance/return act, damage photos, access
log, temperature records, storage invoices. AWB/MAWB/HAWB, airport pair, flight number, terminal/GHA records,
customs и dangerous goods учитываются только как проверяемые факты, ограничения или причины эскалации.

## Область применения

Документ применяется, когда карточка спора подтверждает профиль «аренда ULD, контейнера или терминальной зоны»,
...
признания вины или долга. Для события «return demand, equipment quarantine или airport access issue» Legal
согласует escalation route с operations, finance, insurer, broker или external counsel.

## Термины

- **Аренда uld, контейнера или терминальной зоны** — профиль претензии или письма, который определяет предмет
  доказывания и безопасную позицию.
- **Evidence bundle** — lease order, equipment ID, acceptance/return act, damage photos, access log, temperature
  records, storage invoices; комплект используется для chronology, liability, amount, mitigation и вручения.
- **Без признания** — ответ не подтверждает долг, вину, causation, размер ущерба, waiver of rights или отказ от
  liability cap.
- **Escalation trigger** — return demand, equipment quarantine или airport access issue; событие требует решения
...
- Владелец спора сверяет профиль «аренда ULD, контейнера или терминальной зоны», карточку спора, срок ответа,
  способ вручения и evidence: lease order, equipment ID, acceptance/return act, damage photos, access log,
  temperature records, storage invoices.
...
- Эскалация обязательна при событии: return demand, equipment quarantine или airport access issue. До решения
...
- Сценарий: температурный контейнер возвращен с повреждением двери и потерянным monitoring data.
...
- Подтвердить источниками: AWB/MAWB/HAWB, booking, flight number, cargo manifest, акт терминала или GHA, фото повреждения или расхождения, отметки screening/handling/customs, temperature log при special cargo, invoices/rate sheet, переписка с авиаперевозчиком, GHA, экспедитором, customs broker и страховщиком. Дополнительно: ULD/container handover act, photos, demurrage invoice, temperature record.
...
- Использовать документ как источник типа «претензия контрагенту» по теме «Претензия по аренде ULD, температурного контейнера или терминальной зоны» только при фактическом совпадении запроса с air cargo dispute.
...
- Основные источники: lease order, equipment ID, acceptance/return act, damage photos, access log, temperature
  records, storage invoices.
...
- Проверяемое ограничение: нельзя взыскивать аренду или damage без периода владения, состояния при передаче и
  return record.
...
| Какие факты нужны для «Претензия по аренде ULD, температурного контейнера или терминальной зоны»? | Перечислить недостающие AWB, booking, рейс, airport pair, timeline события, сумму, доказательства вручения и ответственных. |

## Ответственные роли

| Роль | Зона ответственности |
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «аренда ULD, контейнера или терминальной зоны», settlement authority, waiver и спорные liability caps |
| Lease dispute owner | ведет chronology, evidence bundle, срок ответа и коммуникацию по событию: return demand, equipment quarantine или airport access issue |
| Air cargo operations / GHA contact | подтверждает для «Претензия по аренде ULD, температурного контейнера или терминальной зоны» movement, acceptance/offload/release, terminal act, handling records и operational root cause |
| Finance / insurer / broker contact | сверяет по «Претензия по аренде ULD, температурного контейнера или терминальной зоны» сумму требования, reserve, оплату, customs costs, insurance notice и право регресса |

## Риски и ограничения

- Нельзя направлять документ без evidence по профилю «аренда ULD, контейнера или терминальной зоны»: lease
  order, equipment ID, acceptance/return act, damage photos, access log, temperature records, storage invoices.
- Ключевое ограничение: нельзя взыскивать аренду или damage без периода владения, состояния при передаче и
  return record.
...
- ответ относится к профилю «аренда ULD, контейнера или терминальной зоны» и не подменяет его общей
  претензионной процедурой
- доказательства названы предметно: lease order, equipment ID, acceptance/return act, damage photos, access log,
  temperature records, storage invoices
- отдельно указано ограничение или gap: нельзя взыскивать аренду или damage без периода владения, состояния при
  передаче и return record
- срок ответа, способ вручения, escalation route и safe legal position отражены явно

## Контрольный список

- [ ] карточка спора сопоставлена с документом «Претензия по аренде ULD, температурного контейнера или
  терминальной зоны» и профилем «аренда ULD, контейнера или терминальной зоны»
- [ ] собран evidence bundle: lease order, equipment ID, acceptance/return act, damage photos, access log,
  temperature records, storage invoices
- [ ] проверены claim notice, limitation period, proof of delivery/service и authority signer
- [ ] оценено ограничение: нельзя взыскивать аренду или damage без периода владения, состояния при передаче и
  return record
...
Внеплановый пересмотр нужен после события: return demand, equipment quarantine или airport access issue; также
...
При пересмотре Legal проверяет закрытые дела профиля «аренда ULD, контейнера или терминальной зоны»: какие
...
> Применять «Претензия по аренде ULD, температурного контейнера или терминальной зоны» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
