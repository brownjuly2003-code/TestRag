...
Договор поставки оборудования используется для сценария: поставляется оборудование для handling, screening,
weighing, ULD build-up или cold storage. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
technical specification, FAT/SAT plan, airport permit, delivery note, installation act, training record,
warranty terms. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как
условия или ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «поставка cargo terminal equipment» и есть операционная
...
- **Поставка cargo terminal equipment** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — technical specification, FAT/SAT plan, airport permit, delivery note,
  installation act, training record, warranty terms; комплект подтверждает предмет, цену, сроки, приемку и
  пределы ответственности.
- **Операционное ограничение** — оборудование не соответствует airport/security требованиям, integration needs
  или acceptance criteria; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — failed SAT, missing certificate, unsafe operation или delay of terminal launch;
...
- Владелец процесса проверяет профиль «поставка cargo terminal equipment», стороны, authority matrix,
  коммерческие условия и evidence: technical specification, FAT/SAT plan, airport permit, delivery note,
  installation act, training record, warranty terms.
...
- Эскалация обязательна при событии: failed SAT, missing certificate, unsafe operation или delay of terminal
  launch. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «оборудование, поставка, монтаж, гарантия» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор поставки» по теме «Договор поставки оборудования» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: technical specification, FAT/SAT plan, airport permit, delivery note, installation act,
  training record, warranty terms.
...
- Ограничение для ответа: оборудование не соответствует airport/security требованиям, integration needs или
  acceptance criteria. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор поставки оборудования» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «поставка cargo terminal equipment», liability cap, waiver и нестандартные приложения |
| Terminal equipment owner | подтверждает operational facts: technical specification, FAT/SAT plan, airport permit, delivery note, installation act, training record, warranty terms |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор поставки оборудования» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «поставка cargo terminal equipment»: technical
  specification, FAT/SAT plan, airport permit, delivery note, installation act, training record, warranty terms.
- Ключевой риск сценария: оборудование не соответствует airport/security требованиям, integration needs или
  acceptance criteria.
...
- ответ решает именно сценарий «поставка cargo terminal equipment» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: technical specification, FAT/SAT plan, airport permit, delivery
  note, installation act, training record, warranty terms
- учтен профильный риск: оборудование не соответствует airport/security требованиям, integration needs или
  acceptance criteria
...
- [ ] запрос сопоставлен с документом «Договор поставки оборудования» и профилем «поставка cargo terminal
  equipment»
- [ ] собраны профильные evidence: technical specification, FAT/SAT plan, airport permit, delivery note,
  installation act, training record, warranty terms
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: оборудование не соответствует airport/security требованиям, integration needs или
  acceptance criteria
...
Внеплановый пересмотр нужен после события: failed SAT, missing certificate, unsafe operation или delay of
terminal launch; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «поставка cargo terminal equipment» и
...
> Применять «Договор поставки оборудования» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
