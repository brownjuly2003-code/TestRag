...
Договор оказания услуг (общий шаблон) используется для сценария: исполнитель оказывает support по документам,
координации терминала, tracking или back office. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
service order, SLA, task log, AWB/booking references, terminal messages, acceptance acts, incident register.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «операционные услуги для air cargo» и есть операционная
...
- **Операционные услуги для air cargo** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — service order, SLA, task log, AWB/booking references, terminal messages,
  acceptance acts, incident register; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — услуга описана как общий support без границ ответственности, SLA и evidence по
  конкретным shipments; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — missed task, wrong status update, terminal refusal или dispute over service scope;
...
- Владелец процесса проверяет профиль «операционные услуги для air cargo», стороны, authority matrix,
  коммерческие условия и evidence: service order, SLA, task log, AWB/booking references, terminal messages,
  acceptance acts, incident register.
...
- Эскалация обязательна при событии: missed task, wrong status update, terminal refusal или dispute over service
  scope. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «договор услуг, шаблон, неустойка, приемка» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор оказания услуг» по теме «Договор оказания услуг (общий шаблон)» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: service order, SLA, task log, AWB/booking references, terminal messages, acceptance
  acts, incident register.
...
- Ограничение для ответа: услуга описана как общий support без границ ответственности, SLA и evidence по
  конкретным shipments. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор оказания услуг (общий шаблон)» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «операционные услуги для air cargo», liability cap, waiver и нестандартные приложения |
| Service delivery manager | подтверждает operational facts: service order, SLA, task log, AWB/booking references, terminal messages, acceptance acts, incident register |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор оказания услуг (общий шаблон)» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «операционные услуги для air cargo»: service
  order, SLA, task log, AWB/booking references, terminal messages, acceptance acts, incident register.
- Ключевой риск сценария: услуга описана как общий support без границ ответственности, SLA и evidence по
  конкретным shipments.
...
- ответ решает именно сценарий «операционные услуги для air cargo» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: service order, SLA, task log, AWB/booking references, terminal
  messages, acceptance acts, incident register
- учтен профильный риск: услуга описана как общий support без границ ответственности, SLA и evidence по
  конкретным shipments
...
- [ ] запрос сопоставлен с документом «Договор оказания услуг (общий шаблон)» и профилем «операционные услуги
  для air cargo»
- [ ] собраны профильные evidence: service order, SLA, task log, AWB/booking references, terminal messages,
  acceptance acts, incident register
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: услуга описана как общий support без границ ответственности, SLA и evidence по
  конкретным shipments
...
Внеплановый пересмотр нужен после события: missed task, wrong status update, terminal refusal или dispute over
service scope; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «операционные услуги для air cargo» и
...
> Применять «Договор оказания услуг (общий шаблон)» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
