---
doc_id: legal_contract_expedition_general
title: Договор транспортной экспедиции (общий шаблон)
...
- ФЗ от 08.11.2007 № 259-ФЗ (об автомобильном транспорте)
keywords:
- экспедиция
- договор
- перевозка
...
Договор транспортной экспедиции (общий шаблон) используется для сценария: экспедитор организует booking, сдачу в
терминал, tracking и получение авиагруза. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
expedition order, booking, AWB/HAWB, terminal acceptance, pickup/delivery proof, status messages, invoices.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «внутренняя air cargo экспедиция» и есть операционная
связка с air cargo route, terminal handover или расчетами по перевозке.

First/last mile допускается только как поручение экспедитору с отдельным pickup/delivery proof, terminal slot и
связью с AWB/booking. Customs/export control, dangerous goods, temperature-controlled cargo и security screening включаются
только при подтвержденной применимости.

## Термины

- **Внутренняя air cargo экспедиция** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — expedition order, booking, AWB/HAWB, terminal acceptance, pickup/delivery
  proof, status messages, invoices; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — экспедитор обещает результат перевозчика без контроля cutoff, acceptance и
  carrier liability cap; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — missed booking, terminal rejection, shortage on delivery или спор по storage
...
2.1. Предмет: экспедирование авиагруза: booking, HAWB/MAWB, terminal handling, customs/export control, страхование и доставка до/из аэропорта.
...
2.4. First/last mile допускается только как поручение экспедитору с отдельным pickup/delivery proof,
     terminal slot и связью с AWB/booking.
...
3.2. Исполнение подтверждается: MAWB/HAWB, booking confirmation, flight manifest, warehouse receipt, customs declaration, акты терминала, температурные логи и отчеты экспедитора.
...
- Владелец процесса проверяет профиль «внутренняя air cargo экспедиция», стороны, authority matrix, коммерческие
  условия и evidence: expedition order, booking, AWB/HAWB, terminal acceptance, pickup/delivery proof, status
  messages, invoices.
...
- Эскалация обязательна при событии: missed booking, terminal rejection, shortage on delivery или спор по
  storage charges. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Проверить: маршрут, MAWB/HAWB, booking, flight number, аэропорты, chargeable weight, customs/export control, terminal handling и special cargo.
- Подтвердить источниками: MAWB/HAWB, booking confirmation, flight manifest, warehouse receipt, customs declaration, акты терминала, температурные логи и отчеты экспедитора.
...
- Предметная проверка: признаки «экспедиция, договор, перевозка, экспедитор» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор транспортной экспедиции» по теме «Договор транспортной экспедиции (общий шаблон)» только при совпадении запроса с авиагрузовым предметом документа.
- Перед ответом проверить: маршрут, MAWB/HAWB, booking, flight number, аэропорты, chargeable weight, customs/export control, terminal handling и special cargo; полномочия подписантов, маршрут согласования, версию приложений и aviation evidence.
...
- Профильные источники: expedition order, booking, AWB/HAWB, terminal acceptance, pickup/delivery proof, status
  messages, invoices.
...
- Ограничение для ответа: экспедитор обещает результат перевозчика без контроля cutoff, acceptance и carrier
  liability cap. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор транспортной экспедиции (общий шаблон)» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «внутренняя air cargo экспедиция», liability cap, waiver и нестандартные приложения |
| Forwarding operations lead | подтверждает operational facts: expedition order, booking, AWB/HAWB, terminal acceptance, pickup/delivery proof, status messages, invoices |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор транспортной экспедиции (общий шаблон)» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «внутренняя air cargo экспедиция»: expedition
  order, booking, AWB/HAWB, terminal acceptance, pickup/delivery proof, status messages, invoices.
- Ключевой риск сценария: экспедитор обещает результат перевозчика без контроля cutoff, acceptance и carrier
  liability cap.
...
- ответ решает именно сценарий «внутренняя air cargo экспедиция» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: expedition order, booking, AWB/HAWB, terminal acceptance,
  pickup/delivery proof, status messages, invoices
- учтен профильный риск: экспедитор обещает результат перевозчика без контроля cutoff, acceptance и carrier
  liability cap
...
- [ ] запрос сопоставлен с документом «Договор транспортной экспедиции (общий шаблон)» и профилем «внутренняя
  air cargo экспедиция»
- [ ] собраны профильные evidence: expedition order, booking, AWB/HAWB, terminal acceptance, pickup/delivery
  proof, status messages, invoices
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: экспедитор обещает результат перевозчика без контроля cutoff, acceptance и carrier
  liability cap
...
Внеплановый пересмотр нужен после события: missed booking, terminal rejection, shortage on delivery или спор по
storage charges; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «внутренняя air cargo экспедиция» и
...
> Применять «Договор транспортной экспедиции (общий шаблон)» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
