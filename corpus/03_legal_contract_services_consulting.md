---
doc_id: legal_contract_services_consulting
title: Договор на юридическое консультирование
...
- ГК РФ, ст. 779 (договор поручения)
keywords:
- юридические услуги
- консалтинг
- аутсорсинг
...
# Договор на юридическое консультирование

## Назначение

Договор на юридическое консультирование используется для сценария: консультант анализирует маршрут, документы,
customs/export control, liability и claims strategy. Это рабочий шаблон Legal для авиагрузовой сделки, где
...
statement of work, route facts, AWB/booking sample, customs file, SLA, risk memo, deliverable acceptance.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «консалтинг по air cargo legal/customs/SLA» и есть
...
- **Консалтинг по air cargo legal/customs/sla** — профиль документа, который определяет обязательные приложения
  и evidence.
- **Aviation evidence по сделке** — statement of work, route facts, AWB/booking sample, customs file, SLA, risk
  memo, deliverable acceptance; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — консультация подменяет брокерские или перевозочные обязательства и не опирается
  на факты отправки; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — изменение маршрута, dangerous goods issue, denied party risk или спор о
  применимости вывода; после него проект возвращается к Legal, operations и finance/insurance.
...
| `law_firm_name` | Юридическая фирма | `{{law_firm_name}}` |
| `scope_of_services` | Объем услуг | `{{scope_of_services}}` |
| `monthly_fee` | Ежемесячное вознаграждение | `{{monthly_fee}}` |
| `hourly_rate` | Почасовая ставка | `{{hourly_rate}}` |
| `retainer_amount` | Размер ретейнера | `{{retainer_amount}}` |
...
- Юридическая фирма: {{law_firm_name}}
- Объем услуг: {{scope_of_services}}
- Ежемесячное вознаграждение: {{monthly_fee}}
- Почасовая ставка: {{hourly_rate}}
- Размер ретейнера: {{retainer_amount}}
...
- Владелец процесса проверяет профиль «консалтинг по air cargo legal/customs/SLA», стороны, authority matrix,
  коммерческие условия и evidence: statement of work, route facts, AWB/booking sample, customs file, SLA, risk
  memo, deliverable acceptance.
...
- Эскалация обязательна при событии: изменение маршрута, dangerous goods issue, denied party risk или спор о
  применимости вывода. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «юридические услуги, консалтинг, аутсорсинг» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор оказания услуг» по теме «Договор на юридическое консультирование» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: statement of work, route facts, AWB/booking sample, customs file, SLA, risk memo,
...
- Ограничение для ответа: консультация подменяет брокерские или перевозочные обязательства и не опирается на
  факты отправки. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор на юридическое консультирование» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «консалтинг по air cargo legal/customs/SLA», liability cap, waiver и нестандартные приложения |
| Consulting engagement owner | подтверждает operational facts: statement of work, route facts, AWB/booking sample, customs file, SLA, risk memo, deliverable acceptance |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор на юридическое консультирование» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «консалтинг по air cargo legal/customs/SLA»:
  statement of work, route facts, AWB/booking sample, customs file, SLA, risk memo, deliverable acceptance.
- Ключевой риск сценария: консультация подменяет брокерские или перевозочные обязательства и не опирается на
  факты отправки.
...
- ответ решает именно сценарий «консалтинг по air cargo legal/customs/SLA» и не смешивает его со смежным
  договором или претензией
- источники подтверждения названы предметно: statement of work, route facts, AWB/booking sample, customs file,
  SLA, risk memo, deliverable acceptance
- учтен профильный риск: консультация подменяет брокерские или перевозочные обязательства и не опирается на
  факты отправки
...
- [ ] запрос сопоставлен с документом «Договор на юридическое консультирование» и профилем «консалтинг по air
  cargo legal/customs/SLA»
- [ ] собраны профильные evidence: statement of work, route facts, AWB/booking sample, customs file, SLA, risk
  memo, deliverable acceptance
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: консультация подменяет брокерские или перевозочные обязательства и не опирается на
  факты отправки
...
Внеплановый пересмотр нужен после события: изменение маршрута, dangerous goods issue, denied party risk или спор
о применимости вывода; также после изменения carrier/GHA rules, airport security требований, customs/export
control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «консалтинг по air cargo
legal/customs/SLA» и обновляет evidence list, сроки согласования, роли эскалации и ограничения RAG-ответа.
...
> Применять «Договор на юридическое консультирование» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
