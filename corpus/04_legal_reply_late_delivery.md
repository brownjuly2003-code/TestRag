...
title: Ответ на претензию о задержке рейса или offload
...
# Ответ на претензию о задержке рейса или offload

## Назначение

Ответ на претензию о задержке рейса или offload используется для сценария: компания отвечает на требование о
задержке, offload или missed cutoff. Это ответ на претензию для конкретного air cargo dispute, а не общая форма без shipment evidence.

Позиция строится на доказательствах: incoming claim, booking, AWB/HAWB, flight status, offload reason, customs
status, terminal timestamps, mitigation record. AWB/MAWB/HAWB, airport pair, flight number, terminal/GHA
records, customs и dangerous goods учитываются только как проверяемые факты, ограничения или причины эскалации.

## Область применения

Документ применяется, когда карточка спора подтверждает профиль «ответ на delay/offload claim», сторону спора,
...
признания вины или долга. Для события «carrier schedule evidence, customs document request или settlement
review» Legal согласует escalation route с operations, finance, insurer, broker или external counsel.

## Термины

- **Ответ на delay/offload claim** — профиль претензии или письма, который определяет предмет доказывания и
  безопасную позицию.
- **Evidence bundle** — incoming claim, booking, AWB/HAWB, flight status, offload reason, customs status,
  terminal timestamps, mitigation record; комплект используется для chronology, liability, amount, mitigation и
  вручения.
- **Без признания** — ответ не подтверждает долг, вину, causation, размер ущерба, waiver of rights или отказ от
  liability cap.
- **Escalation trigger** — carrier schedule evidence, customs document request или settlement review; событие
...
- Владелец спора сверяет профиль «ответ на delay/offload claim», карточку спора, срок ответа, способ вручения и
  evidence: incoming claim, booking, AWB/HAWB, flight status, offload reason, customs status, terminal
  timestamps, mitigation record.
...
- Эскалация обязательна при событии: carrier schedule evidence, customs document request или settlement review.
  До решения текст не должен признавать долг, вину, causation, сумму ущерба или отказ от прав.
...
- Сценарий: клиент требует штраф, но offload связан с customs hold и late documents shipper.
...
- Подтвердить источниками: AWB/MAWB/HAWB, booking, flight number, cargo manifest, акт терминала или GHA, фото повреждения или расхождения, отметки screening/handling/customs, temperature log при special cargo, invoices/rate sheet, переписка с авиаперевозчиком, GHA, экспедитором, customs broker и страховщиком. Дополнительно: booking history, offload cause, customs hold notice, terminal timestamps, notice log.
...
- Использовать документ как источник типа «ответ на претензию» по теме «Ответ на претензию о задержке рейса или offload» только при фактическом совпадении запроса с air cargo dispute.
...
- Основные источники: incoming claim, booking, AWB/HAWB, flight status, offload reason, customs status, terminal
  timestamps, mitigation record.
...
- Проверяемое ограничение: нельзя признавать просрочку без сравнения planned/actual timeline и исключающих
  причин.
...
| Какие факты нужны для «Ответ на претензию о задержке рейса или offload»? | Перечислить недостающие AWB, booking, рейс, airport pair, timeline события, сумму, доказательства вручения и ответственных. |

## Ответственные роли

| Роль | Зона ответственности |
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «ответ на delay/offload claim», settlement authority, waiver и спорные liability caps |
| Delay reply owner | ведет chronology, evidence bundle, срок ответа и коммуникацию по событию: carrier schedule evidence, customs document request или settlement review |
| Air cargo operations / GHA contact | подтверждает для «Ответ на претензию о задержке рейса или offload» movement, acceptance/offload/release, terminal act, handling records и operational root cause |
| Finance / insurer / broker contact | сверяет по «Ответ на претензию о задержке рейса или offload» сумму требования, reserve, оплату, customs costs, insurance notice и право регресса |

## Риски и ограничения

- Нельзя направлять документ без evidence по профилю «ответ на delay/offload claim»: incoming claim, booking,
  AWB/HAWB, flight status, offload reason, customs status, terminal timestamps, mitigation record.
- Ключевое ограничение: нельзя признавать просрочку без сравнения planned/actual timeline и исключающих причин.
...
- ответ относится к профилю «ответ на delay/offload claim» и не подменяет его общей претензионной процедурой
- доказательства названы предметно: incoming claim, booking, AWB/HAWB, flight status, offload reason, customs
  status, terminal timestamps, mitigation record
- отдельно указано ограничение или gap: нельзя признавать просрочку без сравнения planned/actual timeline и
  исключающих причин
- срок ответа, способ вручения, escalation route и safe legal position отражены явно

## Контрольный список

- [ ] карточка спора сопоставлена с документом «Ответ на претензию о задержке рейса или offload» и профилем
  «ответ на delay/offload claim»
- [ ] собран evidence bundle: incoming claim, booking, AWB/HAWB, flight status, offload reason, customs status,
  terminal timestamps, mitigation record
- [ ] проверены claim notice, limitation period, proof of delivery/service и authority signer
- [ ] оценено ограничение: нельзя признавать просрочку без сравнения planned/actual timeline и исключающих
  причин
...
Внеплановый пересмотр нужен после события: carrier schedule evidence, customs document request или settlement
...
При пересмотре Legal проверяет закрытые дела профиля «ответ на delay/offload claim»: какие evidence сработали,
...
> Применять «Ответ на претензию о задержке рейса или offload» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
