...
Договор о неразглашении (NDA, внешний) используется для сценария: стороны раскрывают тарифы, routes, AWB data,
customer lists и security/customs information. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
NDA request, data room log, tariff sheet, AWB data sample, access list, export control note, recipient
authority. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия
или ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «NDA по авиагрузовым данным» и есть операционная связка с
...
- **Nda по авиагрузовым данным** — профиль документа, который определяет обязательные приложения и evidence.
- **Aviation evidence по сделке** — NDA request, data room log, tariff sheet, AWB data sample, access list,
  export control note, recipient authority; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — конфиденциальные AWB/customs/security данные передаются без need-to-know, срока
  и режима возврата; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — data leak, misuse of rate sheet, disclosure to broker или запрос госоргана; после
...
| `confidential_cargo_data` | Перечень cargo data | `{{confidential_cargo_data}}` |
| `awb_data_scope` | AWB/booking data scope | `{{awb_data_scope}}` |
| `airport_security_scope` | Security-sensitive data | `{{airport_security_scope}}` |
| `data_return_date` | Срок возврата/уничтожения данных | `{{data_return_date}}` |
...
2.1. Предмет: конфиденциальность air cargo data: тарифов, AWB/booking, flight details, shipper/consignee, special cargo, security screening, customs/export control и terminal events.
...
- Перечень cargo data: {{confidential_cargo_data}}
- AWB/booking data scope: {{awb_data_scope}}
- Security-sensitive data: {{airport_security_scope}}
- Срок возврата/уничтожения данных: {{data_return_date}}
...
3.2. Исполнение подтверждается: NDA, реестр cargo data, список доступов, письма о передаче AWB/booking, export control memo и акт уничтожения данных.
...
- Владелец процесса проверяет профиль «NDA по авиагрузовым данным», стороны, authority matrix, коммерческие
  условия и evidence: NDA request, data room log, tariff sheet, AWB data sample, access list, export control
  note, recipient authority.
...
- Эскалация обязательна при событии: data leak, misuse of rate sheet, disclosure to broker или запрос госоргана.
...
- Сценарий: IT-подрядчику передают выгрузку AWB и статусов терминала для настройки WMS/TMS.
- Проверить: категории cargo data, получателей, срок, цель доступа, security/export control ограничения и возврат данных.
- Подтвердить источниками: NDA, реестр cargo data, список доступов, письма о передаче AWB/booking, export control memo и акт уничтожения данных.
...
- Предметная проверка: признаки «NDA, неразглашение, конфиденциальность, коммерческая тайна» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «NDA» по теме «Договор о неразглашении (NDA, внешний)» только при совпадении запроса с авиагрузовым предметом документа.
- Перед ответом проверить: категории cargo data, получателей, срок, цель доступа, security/export control ограничения и возврат данных; полномочия подписантов, маршрут согласования, версию приложений и aviation evidence.
...
- Профильные источники: NDA request, data room log, tariff sheet, AWB data sample, access list, export control
  note, recipient authority.
...
- Ограничение для ответа: конфиденциальные AWB/customs/security данные передаются без need-to-know, срока и
  режима возврата. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор о неразглашении (NDA, внешний)» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «NDA по авиагрузовым данным», liability cap, waiver и нестандартные приложения |
| Information owner | подтверждает operational facts: NDA request, data room log, tariff sheet, AWB data sample, access list, export control note, recipient authority |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор о неразглашении (NDA, внешний)» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «NDA по авиагрузовым данным»: NDA request, data
  room log, tariff sheet, AWB data sample, access list, export control note, recipient authority.
- Ключевой риск сценария: конфиденциальные AWB/customs/security данные передаются без need-to-know, срока и
  режима возврата.
...
- ответ решает именно сценарий «NDA по авиагрузовым данным» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: NDA request, data room log, tariff sheet, AWB data sample, access
  list, export control note, recipient authority
- учтен профильный риск: конфиденциальные AWB/customs/security данные передаются без need-to-know, срока и
  режима возврата
...
- [ ] запрос сопоставлен с документом «Договор о неразглашении (NDA, внешний)» и профилем «NDA по авиагрузовым
  данным»
- [ ] собраны профильные evidence: NDA request, data room log, tariff sheet, AWB data sample, access list,
  export control note, recipient authority
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: конфиденциальные AWB/customs/security данные передаются без need-to-know, срока и
  режима возврата
...
Внеплановый пересмотр нужен после события: data leak, misuse of rate sheet, disclosure to broker или запрос
госоргана; также после изменения carrier/GHA rules, airport security требований, customs/export control режима,
страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «NDA по авиагрузовым данным» и обновляет
...
> Применять «Договор о неразглашении (NDA, внешний)» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
