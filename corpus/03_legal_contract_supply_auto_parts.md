...
Договор поставки автозапчастей используется для сценария: поставляются детали для тягачей, погрузчиков, весов,
cold chain или terminal equipment. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется по
...
purchase order, specs, serial numbers, conformity documents, delivery note, acceptance act, defect report.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «поставка запчастей для GSE/терминала» и есть операционная
...
- **Поставка запчастей для gse/терминала** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — purchase order, specs, serial numbers, conformity documents, delivery note,
  acceptance act, defect report; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — запчасть не совместима с airport equipment, warranty, safety или maintenance
  schedule; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — equipment downtime, wrong part, warranty rejection или delayed delivery before
  peak flight window; после него проект возвращается к Legal, operations и finance/insurance.
...
| `price_list` | Прайс-лист | `{{price_list}}` |
...
- Владелец процесса проверяет профиль «поставка запчастей для GSE/терминала», стороны, authority matrix,
  коммерческие условия и evidence: purchase order, specs, serial numbers, conformity documents, delivery note,
  acceptance act, defect report.
...
- Эскалация обязательна при событии: equipment downtime, wrong part, warranty rejection или delayed delivery
  before peak flight window. Решение фиксируется в версии договора, приложении, протоколе разногласий или change
  log.
...
- Предметная проверка: признаки «автозапчасти, поставка, автопарк» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор поставки» по теме «Договор поставки автозапчастей» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: purchase order, specs, serial numbers, conformity documents, delivery note, acceptance
  act, defect report.
...
- Ограничение для ответа: запчасть не совместима с airport equipment, warranty, safety или maintenance schedule.
...
|---------------------|-------------------------------|
| Подходит ли «Договор поставки автозапчастей» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «поставка запчастей для GSE/терминала», liability cap, waiver и нестандартные приложения |
| Maintenance supply owner | подтверждает operational facts: purchase order, specs, serial numbers, conformity documents, delivery note, acceptance act, defect report |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор поставки автозапчастей» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «поставка запчастей для GSE/терминала»: purchase
  order, specs, serial numbers, conformity documents, delivery note, acceptance act, defect report.
- Ключевой риск сценария: запчасть не совместима с airport equipment, warranty, safety или maintenance schedule.
...
- ответ решает именно сценарий «поставка запчастей для GSE/терминала» и не смешивает его со смежным договором
  или претензией
- источники подтверждения названы предметно: purchase order, specs, serial numbers, conformity documents,
  delivery note, acceptance act, defect report
- учтен профильный риск: запчасть не совместима с airport equipment, warranty, safety или maintenance schedule
...
- [ ] запрос сопоставлен с документом «Договор поставки автозапчастей» и профилем «поставка запчастей для
  GSE/терминала»
- [ ] собраны профильные evidence: purchase order, specs, serial numbers, conformity documents, delivery note,
  acceptance act, defect report
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: запчасть не совместима с airport equipment, warranty, safety или maintenance schedule
...
Внеплановый пересмотр нужен после события: equipment downtime, wrong part, warranty rejection или delayed
delivery before peak flight window; также после изменения carrier/GHA rules, airport security требований,
customs/export control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «поставка запчастей для GSE/терминала» и
...
> Применять «Договор поставки автозапчастей» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
