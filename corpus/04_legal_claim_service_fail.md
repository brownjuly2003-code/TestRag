...
title: Претензия по ненадлежащему screening, handling или terminal service
...
# Претензия по ненадлежащему screening, handling или terminal service

## Назначение

Претензия по ненадлежащему screening, handling или terminal service используется для сценария: услуга screening,
build-up, weighing, storage или release выполнена с ошибкой. Это претензия контрагенту для конкретного air cargo
dispute, а не общая форма без shipment evidence.

Позиция строится на доказательствах: service order, terminal/GHA act, screening record, weight note, CCTV
request, photos, incident report, correspondence. AWB/MAWB/HAWB, airport pair, flight number, terminal/GHA
records, customs и dangerous goods учитываются только как проверяемые факты, ограничения или причины эскалации.

## Область применения

Документ применяется, когда карточка спора подтверждает профиль «ненадлежащий screening/handling/terminal
service», сторону спора, сумму или требование, срок ответа и способ вручения.

Если не хватает AWB/booking, terminal act, customs record или proof of delivery, готовится запрос evidence без
признания вины или долга. Для события «GHA investigation, corrective action request или carrier escalation»
Legal согласует escalation route с operations, finance, insurer, broker или external counsel.

## Термины

- **Ненадлежащий screening/handling/terminal service** — профиль претензии или письма, который определяет
  предмет доказывания и безопасную позицию.
- **Evidence bundle** — service order, terminal/GHA act, screening record, weight note, CCTV request, photos,
  incident report, correspondence; комплект используется для chronology, liability, amount, mitigation и
  вручения.
- **Без признания** — ответ не подтверждает долг, вину, causation, размер ущерба, waiver of rights или отказ от
  liability cap.
- **Escalation trigger** — GHA investigation, corrective action request или carrier escalation; событие требует
...
- Владелец спора сверяет профиль «ненадлежащий screening/handling/terminal service», карточку спора, срок
  ответа, способ вручения и evidence: service order, terminal/GHA act, screening record, weight note, CCTV
  request, photos, incident report, correspondence.
...
- Эскалация обязательна при событии: GHA investigation, corrective action request или carrier escalation. До
...
- Сценарий: GHA отказал в acceptance dangerous goods после cutoff из-за late DGD и labels.
...
- Подтвердить источниками: AWB/MAWB/HAWB, booking, flight number, cargo manifest, акт терминала или GHA, фото повреждения или расхождения, отметки screening/handling/customs, temperature log при special cargo, invoices/rate sheet, переписка с авиаперевозчиком, GHA, экспедитором, customs broker и страховщиком. Дополнительно: DGD/MSDS, screening log, terminal rejection act, photos of labels, SLA.
...
- Использовать документ как источник типа «претензия контрагенту» по теме «Претензия по ненадлежащему screening, handling или terminal service» только при фактическом совпадении запроса с air cargo dispute.
...
- Основные источники: service order, terminal/GHA act, screening record, weight note, CCTV request, photos,
...
- Проверяемое ограничение: нельзя заявлять service failure без service order, timestamp и связи с ущербом или
  задержкой.
...
| Какие факты нужны для «Претензия по ненадлежащему screening, handling или terminal service»? | Перечислить недостающие AWB, booking, рейс, airport pair, timeline события, сумму, доказательства вручения и ответственных. |

## Ответственные роли

| Роль | Зона ответственности |
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «ненадлежащий screening/handling/terminal service», settlement authority, waiver и спорные liability caps |
| Terminal service claim owner | ведет chronology, evidence bundle, срок ответа и коммуникацию по событию: GHA investigation, corrective action request или carrier escalation |
| Air cargo operations / GHA contact | подтверждает для «Претензия по ненадлежащему screening, handling или terminal service» movement, acceptance/offload/release, terminal act, handling records и operational root cause |
| Finance / insurer / broker contact | сверяет по «Претензия по ненадлежащему screening, handling или terminal service» сумму требования, reserve, оплату, customs costs, insurance notice и право регресса |

## Риски и ограничения

- Нельзя направлять документ без evidence по профилю «ненадлежащий screening/handling/terminal service»: service
  order, terminal/GHA act, screening record, weight note, CCTV request, photos, incident report, correspondence.
- Ключевое ограничение: нельзя заявлять service failure без service order, timestamp и связи с ущербом или
  задержкой.
...
- ответ относится к профилю «ненадлежащий screening/handling/terminal service» и не подменяет его общей
  претензионной процедурой
- доказательства названы предметно: service order, terminal/GHA act, screening record, weight note, CCTV
  request, photos, incident report, correspondence
- отдельно указано ограничение или gap: нельзя заявлять service failure без service order, timestamp и связи с
  ущербом или задержкой
- срок ответа, способ вручения, escalation route и safe legal position отражены явно

## Контрольный список

- [ ] карточка спора сопоставлена с документом «Претензия по ненадлежащему screening, handling или terminal
  service» и профилем «ненадлежащий screening/handling/terminal service»
- [ ] собран evidence bundle: service order, terminal/GHA act, screening record, weight note, CCTV request,
  photos, incident report, correspondence
- [ ] проверены claim notice, limitation period, proof of delivery/service и authority signer
- [ ] оценено ограничение: нельзя заявлять service failure без service order, timestamp и связи с ущербом или
  задержкой
...
Внеплановый пересмотр нужен после события: GHA investigation, corrective action request или carrier escalation;
...
При пересмотре Legal проверяет закрытые дела профиля «ненадлежащий screening/handling/terminal service»: какие
evidence сработали, какие сроки были сорваны, где RAG-ответ требовал safe-position предупреждения.
...
> Применять «Претензия по ненадлежащему screening, handling или terminal service» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
