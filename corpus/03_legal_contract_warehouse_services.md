---
doc_id: legal_contract_warehouse_services
title: Договор на складские услуги
...
# Договор на складские услуги

## Назначение

Договор на складские услуги используется для сценария: склад оказывает acceptance, storage, screening support,
release, фотофиксацию и status reporting. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
service order, terminal acceptance, cargo manifest, storage records, temperature log, release note, damage
report. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «услуги cargo terminal warehouse» и есть операционная
...
- **Услуги cargo terminal warehouse** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — service order, terminal acceptance, cargo manifest, storage records,
  temperature log, release note, damage report; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — warehouse service не фиксирует custody transfer, temperature, DG segregation
  или release authority; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — shortage, damage during storage, customs hold, release to wrong consignee или
  storage overrun; после него проект возвращается к Legal, operations и finance/insurance.
...
| `service_provider_name` | Наименование поставщика услуг | `{{service_provider_name}}` |
| `services_list` | Перечень услуг | `{{services_list}}` |
| `throughput_volume` | Объем пропускной способности | `{{throughput_volume}}` |
| `service_fee` | Стоимость услуги | `{{service_fee}}` |
...
- Наименование поставщика услуг: {{service_provider_name}}
- Перечень услуг: {{services_list}}
- Объем пропускной способности: {{throughput_volume}}
- Стоимость услуги: {{service_fee}}
...
- Владелец процесса проверяет профиль «услуги cargo terminal warehouse», стороны, authority matrix, коммерческие
  условия и evidence: service order, terminal acceptance, cargo manifest, storage records, temperature log,
  release note, damage report.
...
- Эскалация обязательна при событии: shortage, damage during storage, customs hold, release to wrong consignee
  или storage overrun. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «складские услуги, 3PL, логистика» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор оказания услуг» по теме «Договор на складские услуги» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: service order, terminal acceptance, cargo manifest, storage records, temperature log,
  release note, damage report.
...
- Ограничение для ответа: warehouse service не фиксирует custody transfer, temperature, DG segregation или
  release authority. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор на складские услуги» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «услуги cargo terminal warehouse», liability cap, waiver и нестандартные приложения |
| Terminal warehouse supervisor | подтверждает operational facts: service order, terminal acceptance, cargo manifest, storage records, temperature log, release note, damage report |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор на складские услуги» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «услуги cargo terminal warehouse»: service
  order, terminal acceptance, cargo manifest, storage records, temperature log, release note, damage report.
- Ключевой риск сценария: warehouse service не фиксирует custody transfer, temperature, DG segregation или
  release authority.
...
- ответ решает именно сценарий «услуги cargo terminal warehouse» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: service order, terminal acceptance, cargo manifest, storage
  records, temperature log, release note, damage report
- учтен профильный риск: warehouse service не фиксирует custody transfer, temperature, DG segregation или
  release authority
...
- [ ] запрос сопоставлен с документом «Договор на складские услуги» и профилем «услуги cargo terminal warehouse»
- [ ] собраны профильные evidence: service order, terminal acceptance, cargo manifest, storage records,
  temperature log, release note, damage report
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: warehouse service не фиксирует custody transfer, temperature, DG segregation или
  release authority
...
Внеплановый пересмотр нужен после события: shortage, damage during storage, customs hold, release to wrong
consignee или storage overrun; также после изменения carrier/GHA rules, airport security требований,
customs/export control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «услуги cargo terminal warehouse» и
...
> Применять «Договор на складские услуги» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
