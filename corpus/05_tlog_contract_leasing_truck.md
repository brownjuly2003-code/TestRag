...
title: Договор лизинга транспорта для airport first/last mile
...
- лизинг
- грузовик
- ТС наземного аэропортового плеча first/last mile
- выкуп
confidentiality: internal
---

# Договор лизинга транспорта для airport first/last mile

## Назначение

Шаблон лизинга транспорта для airport first/last mile: подвоз/вывоз авиагруза до/из терминала, контроль пропусков, пломб, GPS и temperature logger. Используется в авиагрузовой логистике ООО «Северный Контур» для проверки booking, AWB/MAWB/HAWB, terminal acceptance, screening, ULD/pallet records и акты; эскалация в юридический отдел происходит при damage/loss, offload, missed cutoff, customs/security hold, temperature excursion, terminal charges или insurance regress.
...
- **Рабочий экземпляр «Договор лизинга транспорта для airport first/last mile»** — версия документа с датой, владельцем процесса, статусом согласования и приложениями, по которой можно принимать внутреннее решение.
- **Основание применения «Договор лизинга транспорта для airport first/last mile»** — заявка, приказ, договор, акт, обращение или иной первичный документ, без которого ассистент не должен формировать окончательный вывод.
...
- Владелец процесса проверяет, что «Договор лизинга транспорта для airport first/last mile» применяется к
  подтвержденной air cargo операции, а не к общей перевозке без booking, AWB/MAWB/HAWB и terminal record.
- Рабочий комплект: leased vehicle, airport access, insurance, maintenance records и use limits for
  first/last mile. CMR/ТТН, GPS и данные водителя используются только для наземного подвоза или вывоза, если
  этот участок связан с airport pair и рейсом.
- Срок реакции: контроль выполняется до выпуска ТС на терминальный маршрут; missed cutoff, failed screening,
...
- Основания для HOLD/STOP: нет полиса, maintenance record, airport access или ограничения на перевозку
  спецгруза. Неподтвержденные даты, суммы, персональные данные, пломбы, вес, temperature mode и реквизиты не
  подставляются.
- Маршрут эскалации: fleet owner, logistics, QHSE, finance и legal; спорный RAG-ответ, проект письма или
...
- Предметная проверка: признаки «лизинг транспорта airport first/last mile, terminal pass, seal/GPS, выкуп» сверяются с исходными документами и не используются как общий ярлык.

## Air cargo профиль документа

- Для «Договор лизинга транспорта для airport first/last mile» air cargo профиль строится вокруг
...
- Предметная опора документа: leased vehicle, airport access, insurance, maintenance records и use limits
  for first/last mile. Эти факты должны быть видны в TMS/WMS/terminal log, акте, фото, переписке carrier/GSA
...
- Special cargo требует ручной проверки для dangerous goods, lithium batteries, pharma/cold chain,
  perishables, oversized и valuable cargo; HOLD наступает, если нет полиса, maintenance record, airport
  access или ограничения на перевозку спецгруза.
- Эскалация идет через fleet owner, logistics, QHSE, finance и legal, чтобы документ не превращался в общий
  транспортный шаблон без авиационных доказательств.
## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор лизинга транспорта для airport first/last mile» только при фактическом совпадении запроса с предметом документа.
...
- Если фактов по «Договор лизинга транспорта для airport first/last mile» не хватает, вернуть уточняющие вопросы; даты, суммы, доступы и реквизиты оставлять неподтвержденными до проверки.
...
- ключевые признаки документа: лизинг транспорта airport first/last mile, terminal pass, seal/GPS, выкуп.
...
| Какие факты нужны для «Договор лизинга транспорта для airport first/last mile»? | Перечислить недостающие даты, документы, участников, суммы/сроки и ответственных. |
| Можно ли применять автоматически? | Только после проверки ограничений по «Договор лизинга транспорта для airport first/last mile»; при риске или неполных фактах нужна ручная проверка. |
...
| Терминал / склад / наземное плечо | передает для «Договор лизинга транспорта для airport first/last mile» booking, AWB, terminal records, CMR/ТТН и GPS только как first/last mile evidence |
...
- ключевые признаки (лизинг транспорта airport first/last mile, terminal pass, seal/GPS, выкуп) могут быть неверно применены без документа-основания.
...
- ответ решает именно сценарий «Договор лизинга транспорта для airport first/last mile» и не подменяет его общим правилом категории.
...
- ключевые признаки (лизинг транспорта airport first/last mile, terminal pass, seal/GPS, выкуп) использованы только вместе с подтвержденными документами.
...
- [ ] запрос сопоставлен с документом «Договор лизинга транспорта для airport first/last mile» и типом «договор»
...
- [ ] недостающие факты по «Договор лизинга транспорта для airport first/last mile» вынесены в уточняющие вопросы
...
> Применять «Договор лизинга транспорта для airport first/last mile» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
