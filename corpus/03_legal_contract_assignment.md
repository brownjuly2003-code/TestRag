...
Договор цессии (уступки права требования) используется для сценария: передается денежное требование по freight,
handling, storage, customs service или страховой выплате. Это рабочий шаблон Legal для авиагрузовой сделки, где
...
договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов, notice debtor, claim file. AWB/MAWB/HAWB,
airport, flight, terminal, customs и dangerous goods указываются только как условия или ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «уступка авиагрузового требования» и есть операционная
...
- **Уступка авиагрузового требования** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов,
  notice debtor, claim file; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — уступается спорное требование без подтверждения объема, должника, зачета или
  лимита; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — возражение должника, частичная оплата, страховой регресс или спор по chargeable
  weight; после него проект возвращается к Legal, operations и finance/insurance.
...
| `assignment_price` | Цена уступки | `{{assignment_price}}` |
...
- Владелец процесса проверяет профиль «уступка авиагрузового требования», стороны, authority matrix,
  коммерческие условия и evidence: договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов, notice
  debtor, claim file.
...
- Эскалация обязательна при событии: возражение должника, частичная оплата, страховой регресс или спор по
  chargeable weight. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «цессия, уступка, дебиторка, факторинг» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор цессии (уступки права требования)» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов, notice debtor,
  claim file.
...
- Ограничение для ответа: уступается спорное требование без подтверждения объема, должника, зачета или лимита.
...
|---------------------|-------------------------------|
| Подходит ли «Договор цессии (уступки права требования)» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «уступка авиагрузового требования», liability cap, waiver и нестандартные приложения |
| Finance receivables owner | подтверждает operational facts: договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов, notice debtor, claim file |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор цессии (уступки права требования)» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «уступка авиагрузового требования»:
  договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов, notice debtor, claim file.
- Ключевой риск сценария: уступается спорное требование без подтверждения объема, должника, зачета или лимита.
...
- ответ решает именно сценарий «уступка авиагрузового требования» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка
  расчетов, notice debtor, claim file
- учтен профильный риск: уступается спорное требование без подтверждения объема, должника, зачета или лимита
...
- [ ] запрос сопоставлен с документом «Договор цессии (уступки права требования)» и профилем «уступка
  авиагрузового требования»
- [ ] собраны профильные evidence: договор-основание, AWB/MAWB/HAWB, invoice, акт услуг, сверка расчетов, notice
  debtor, claim file
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: уступается спорное требование без подтверждения объема, должника, зачета или лимита
...
Внеплановый пересмотр нужен после события: возражение должника, частичная оплата, страховой регресс или спор по
chargeable weight; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «уступка авиагрузового требования» и
...
> Применять «Договор цессии (уступки права требования)» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
