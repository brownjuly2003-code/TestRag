---
doc_id: legal_reply_defective_goods
title: Ответ на претензию по temperature excursion авиагруза
...
# Ответ на претензию по temperature excursion авиагруза

## Назначение

Ответ на претензию по temperature excursion авиагруза используется для сценария: компания отвечает на претензию
о порче или дефекте special cargo. Это ответ на претензию для конкретного air cargo dispute, а не общая форма без shipment evidence.

Позиция строится на доказательствах: incoming claim, temperature logger report, handling instruction, AWB/HAWB,
terminal records, quality report, photos. AWB/MAWB/HAWB, airport pair, flight number, terminal/GHA records,
customs и dangerous goods учитываются только как проверяемые факты, ограничения или причины эскалации.

## Область применения

Документ применяется, когда карточка спора подтверждает профиль «ответ на temperature excursion claim», сторону
...
признания вины или долга. Для события «joint inspection, quarantine release или insurer reservation» Legal
согласует escalation route с operations, finance, insurer, broker или external counsel.

## Термины

- **Ответ на temperature excursion claim** — профиль претензии или письма, который определяет предмет
  доказывания и безопасную позицию.
- **Evidence bundle** — incoming claim, temperature logger report, handling instruction, AWB/HAWB, terminal
  records, quality report, photos; комплект используется для chronology, liability, amount, mitigation и
  вручения.
- **Без признания** — ответ не подтверждает долг, вину, causation, размер ущерба, waiver of rights или отказ от
  liability cap.
- **Escalation trigger** — joint inspection, quarantine release или insurer reservation; событие требует решения
...
- Владелец спора сверяет профиль «ответ на temperature excursion claim», карточку спора, срок ответа, способ
  вручения и evidence: incoming claim, temperature logger report, handling instruction, AWB/HAWB, terminal
  records, quality report, photos.
...
- Эскалация обязательна при событии: joint inspection, quarantine release или insurer reservation. До решения
...
- Сценарий: получатель заявил порчу pharma cargo без data logger report.
...
- Подтвердить источниками: AWB/MAWB/HAWB, booking, flight number, cargo manifest, акт терминала или GHA, фото повреждения или расхождения, отметки screening/handling/customs, temperature log при special cargo, invoices/rate sheet, переписка с авиаперевозчиком, GHA, экспедитором, customs broker и страховщиком. Дополнительно: incoming claim, data logger request, special cargo instructions, GHA temperature records.
...
- Использовать документ как источник типа «ответ на претензию» по теме «Ответ на претензию по temperature excursion авиагруза» только при фактическом совпадении запроса с air cargo dispute.
...
- Основные источники: incoming claim, temperature logger report, handling instruction, AWB/HAWB, terminal
  records, quality report, photos.
...
- Проверяемое ограничение: нельзя признавать дефект перевозочным без logger chain, packaging review и condition
  at release.
...
| Какие факты нужны для «Ответ на претензию по temperature excursion авиагруза»? | Перечислить недостающие AWB, booking, рейс, airport pair, timeline события, сумму, доказательства вручения и ответственных. |

## Ответственные роли

| Роль | Зона ответственности |
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «ответ на temperature excursion claim», settlement authority, waiver и спорные liability caps |
| Quality reply owner | ведет chronology, evidence bundle, срок ответа и коммуникацию по событию: joint inspection, quarantine release или insurer reservation |
| Air cargo operations / GHA contact | подтверждает для «Ответ на претензию по temperature excursion авиагруза» movement, acceptance/offload/release, terminal act, handling records и operational root cause |
| Finance / insurer / broker contact | сверяет по «Ответ на претензию по temperature excursion авиагруза» сумму требования, reserve, оплату, customs costs, insurance notice и право регресса |

## Риски и ограничения

- Нельзя направлять документ без evidence по профилю «ответ на temperature excursion claim»: incoming claim,
  temperature logger report, handling instruction, AWB/HAWB, terminal records, quality report, photos.
- Ключевое ограничение: нельзя признавать дефект перевозочным без logger chain, packaging review и condition at
  release.
...
- ответ относится к профилю «ответ на temperature excursion claim» и не подменяет его общей претензионной
  процедурой
- доказательства названы предметно: incoming claim, temperature logger report, handling instruction, AWB/HAWB,
  terminal records, quality report, photos
- отдельно указано ограничение или gap: нельзя признавать дефект перевозочным без logger chain, packaging review
  и condition at release
- срок ответа, способ вручения, escalation route и safe legal position отражены явно

## Контрольный список

- [ ] карточка спора сопоставлена с документом «Ответ на претензию по temperature excursion авиагруза» и
  профилем «ответ на temperature excursion claim»
- [ ] собран evidence bundle: incoming claim, temperature logger report, handling instruction, AWB/HAWB,
  terminal records, quality report, photos
- [ ] проверены claim notice, limitation period, proof of delivery/service и authority signer
- [ ] оценено ограничение: нельзя признавать дефект перевозочным без logger chain, packaging review и condition
  at release
...
Внеплановый пересмотр нужен после события: joint inspection, quarantine release или insurer reservation; также
...
При пересмотре Legal проверяет закрытые дела профиля «ответ на temperature excursion claim»: какие evidence
...
> Применять «Ответ на претензию по temperature excursion авиагруза» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
