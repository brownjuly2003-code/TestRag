---
doc_id: legal_contract_penalty_clause
title: Регламент применения неустоек и штрафов
...
# Регламент применения неустоек и штрафов

## Назначение

Регламент применения неустоек и штрафов используется для сценария: фиксируются penalties за missed cutoff,
offload, terminal delay, storage overrun или документарные ошибки. Это рабочий шаблон Legal для авиагрузовой
...
SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp, delay reason, calculation sheet.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «неустойка за air cargo SLA» и есть операционная связка с
...
- **Неустойка за air cargo sla** — профиль документа, который определяет обязательные приложения и evidence.
- **Aviation evidence по сделке** — SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp, delay
  reason, calculation sheet; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — штраф считается без causal link, исключений перевозчика, liability cap или
  доказанного timestamp; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — disputed penalty, force majeure claim, customs hold или carrier schedule change;
...
| `penalty_calculation` | Расчет неустойки | `{{penalty_calculation}}` |
...
| `breached_sla_metric` | Нарушенный air cargo SLA | `{{breached_sla_metric}}` |
| `awb_booking_ref` | AWB/booking | `{{awb_booking_ref}}` |
| `incident_report_ref` | Акт события терминала | `{{incident_report_ref}}` |
| `liability_cap` | Лимит ответственности | `{{liability_cap}}` |
...
2.1. Предмет: расчет неустоек за late booking, missed cutoff, delayed handling, terminal damage, shortage, temperature excursion, customs delay или отказ в приемке.
2.2. Существенные параметры сделки:
- Ссылка на договор: {{contract_ref}}
- Описание нарушения: {{breach_description}}
- Расчет неустойки: {{penalty_calculation}}
- Сумма требования: {{claim_amount}}
- Правовое основание: {{legal_basis}}
- Утвердившее лицо: {{approved_by}}
- Нарушенный air cargo SLA: {{breached_sla_metric}}
- AWB/booking: {{awb_booking_ref}}
- Акт события терминала: {{incident_report_ref}}
- Лимит ответственности: {{liability_cap}}
...
3.2. Исполнение подтверждается: AWB/booking, SLA report, terminal incident report, temperature log, flight status, customs notice, расчет штрафа и претензионная переписка.
...
- Владелец процесса проверяет профиль «неустойка за air cargo SLA», стороны, authority matrix, коммерческие
  условия и evidence: SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp, delay reason,
  calculation sheet.
...
- Эскалация обязательна при событии: disputed penalty, force majeure claim, customs hold или carrier schedule
  change. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Сценарий: клиент требует penalty за missed cutoff и temperature excursion по pharma cargo.
- Проверить: SLA, факт нарушения, AWB/booking, причинную связь, исключения, liability cap, страхование и расчет.
- Подтвердить источниками: AWB/booking, SLA report, terminal incident report, temperature log, flight status, customs notice, расчет штрафа и претензионная переписка.
...
- Предметная проверка: признаки «неустойка, штраф, пеня, взыскание» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «регламент» по теме «Регламент применения неустоек и штрафов» только при совпадении запроса с авиагрузовым предметом документа.
- Перед ответом проверить: SLA, факт нарушения, AWB/booking, причинную связь, исключения, liability cap, страхование и расчет; полномочия подписантов, маршрут согласования, версию приложений и aviation evidence.
...
- Профильные источники: SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp, delay reason,
  calculation sheet.
...
- Ограничение для ответа: штраф считается без causal link, исключений перевозчика, liability cap или доказанного
  timestamp. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей договорной
  нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Регламент применения неустоек и штрафов» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «неустойка за air cargo SLA», liability cap, waiver и нестандартные приложения |
| SLA owner | подтверждает operational facts: SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp, delay reason, calculation sheet |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Регламент применения неустоек и штрафов» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «неустойка за air cargo SLA»: SLA appendix,
  booking, AWB/HAWB, flight status, terminal timestamp, delay reason, calculation sheet.
- Ключевой риск сценария: штраф считается без causal link, исключений перевозчика, liability cap или доказанного
  timestamp.
...
- ответ решает именно сценарий «неустойка за air cargo SLA» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp,
  delay reason, calculation sheet
- учтен профильный риск: штраф считается без causal link, исключений перевозчика, liability cap или доказанного
  timestamp
...
- [ ] запрос сопоставлен с документом «Регламент применения неустоек и штрафов» и профилем «неустойка за air
  cargo SLA»
- [ ] собраны профильные evidence: SLA appendix, booking, AWB/HAWB, flight status, terminal timestamp, delay
  reason, calculation sheet
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: штраф считается без causal link, исключений перевозчика, liability cap или
  доказанного timestamp
...
Внеплановый пересмотр нужен после события: disputed penalty, force majeure claim, customs hold или carrier
schedule change; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «неустойка за air cargo SLA» и обновляет
...
> Применять «Регламент применения неустоек и штрафов» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
