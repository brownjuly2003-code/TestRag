...
title: Уведомление об арбитраже по авиагрузовому спору
...
# Уведомление об арбитраже по авиагрузовому спору

## Назначение

Уведомление об арбитраже по авиагрузовому спору используется для сценария: сторона запускает dispute resolution
по air cargo claim после претензионного этапа. Это уведомление для конкретного air cargo dispute, а не общая форма без shipment evidence.

Позиция строится на доказательствах: arbitration clause, claim file, proof of notice, AWB/HAWB, calculation,
settlement history, authority documents. AWB/MAWB/HAWB, airport pair, flight number, terminal/GHA records,
customs и dangerous goods учитываются только как проверяемые факты, ограничения или причины эскалации.

## Область применения

Документ применяется, когда карточка спора подтверждает профиль «уведомление об арбитраже», сторону спора, сумму
...
признания вины или долга. Для события «arbitration filing deadline или settlement standstill» Legal согласует
escalation route с operations, finance, insurer, broker или external counsel.

## Термины

- **Уведомление об арбитраже** — профиль претензии или письма, который определяет предмет доказывания и
  безопасную позицию.
- **Evidence bundle** — arbitration clause, claim file, proof of notice, AWB/HAWB, calculation, settlement
  history, authority documents; комплект используется для chronology, liability, amount, mitigation и вручения.
- **Без признания** — ответ не подтверждает долг, вину, causation, размер ущерба, waiver of rights или отказ от
  liability cap.
- **Escalation trigger** — arbitration filing deadline или settlement standstill; событие требует решения по
...
- Владелец спора сверяет профиль «уведомление об арбитраже», карточку спора, срок ответа, способ вручения и
  evidence: arbitration clause, claim file, proof of notice, AWB/HAWB, calculation, settlement history,
  authority documents.
...
- Эскалация обязательна при событии: arbitration filing deadline или settlement standstill. До решения текст не
...
- Сценарий: carrier не ответил на претензию по terminal damage, и компания уведомляет об арбитраже.
...
- Подтвердить источниками: AWB/MAWB/HAWB, booking, flight number, cargo manifest, акт терминала или GHA, фото повреждения или расхождения, отметки screening/handling/customs, temperature log при special cargo, invoices/rate sheet, переписка с авиаперевозчиком, GHA, экспедитором, customs broker и страховщиком. Дополнительно: arbitration clause, proof of delivery, claim bundle, evidence preservation notice.
...
- Использовать документ как источник типа «уведомление» по теме «Уведомление об арбитраже по авиагрузовому спору» только при фактическом совпадении запроса с air cargo dispute.
...
- Основные источники: arbitration clause, claim file, proof of notice, AWB/HAWB, calculation, settlement
  history, authority documents.
...
- Проверяемое ограничение: нельзя направлять notice без проверки forum, notice method, limitation period и
  полномочий.
...
| Какие факты нужны для «Уведомление об арбитраже по авиагрузовому спору»? | Перечислить недостающие AWB, booking, рейс, airport pair, timeline события, сумму, доказательства вручения и ответственных. |

## Ответственные роли

| Роль | Зона ответственности |
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «уведомление об арбитраже», settlement authority, waiver и спорные liability caps |
| Dispute resolution counsel | ведет chronology, evidence bundle, срок ответа и коммуникацию по событию: arbitration filing deadline или settlement standstill |
| Air cargo operations / GHA contact | подтверждает для «Уведомление об арбитраже по авиагрузовому спору» movement, acceptance/offload/release, terminal act, handling records и operational root cause |
| Finance / insurer / broker contact | сверяет по «Уведомление об арбитраже по авиагрузовому спору» сумму требования, reserve, оплату, customs costs, insurance notice и право регресса |

## Риски и ограничения

- Нельзя направлять документ без evidence по профилю «уведомление об арбитраже»: arbitration clause, claim file,
  proof of notice, AWB/HAWB, calculation, settlement history, authority documents.
- Ключевое ограничение: нельзя направлять notice без проверки forum, notice method, limitation period и
  полномочий.
...
- ответ относится к профилю «уведомление об арбитраже» и не подменяет его общей претензионной процедурой
- доказательства названы предметно: arbitration clause, claim file, proof of notice, AWB/HAWB, calculation,
  settlement history, authority documents
- отдельно указано ограничение или gap: нельзя направлять notice без проверки forum, notice method, limitation
  period и полномочий
- срок ответа, способ вручения, escalation route и safe legal position отражены явно

## Контрольный список

- [ ] карточка спора сопоставлена с документом «Уведомление об арбитраже по авиагрузовому спору» и профилем
  «уведомление об арбитраже»
- [ ] собран evidence bundle: arbitration clause, claim file, proof of notice, AWB/HAWB, calculation, settlement
  history, authority documents
- [ ] проверены claim notice, limitation period, proof of delivery/service и authority signer
- [ ] оценено ограничение: нельзя направлять notice без проверки forum, notice method, limitation period и
  полномочий
...
Внеплановый пересмотр нужен после события: arbitration filing deadline или settlement standstill; также после
нового claim pattern, изменения carrier/GHA rules, customs practice, insurance wording, limitation period или
судебной практики.

При пересмотре Legal проверяет закрытые дела профиля «уведомление об арбитраже»: какие evidence сработали, какие
...
> Применять «Уведомление об арбитраже по авиагрузовому спору» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
