...
Договор ответственного хранения используется для сценария: хранитель принимает авиагруз по местам, весу, режиму
и terminal release instructions. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется по
...
storage receipt, AWB/HAWB, cargo manifest, condition photos, temperature log, access log, release instruction.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «ответственное хранение авиагруза» и есть операционная
...
- **Ответственное хранение авиагруза** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — storage receipt, AWB/HAWB, cargo manifest, condition photos, temperature
  log, access log, release instruction; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — хранение не подтверждает состояние груза, custody, special cargo режим или
  полномочия на выдачу; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — loss from storage, temperature excursion, unauthorized release или inventory
...
| `storage_fee` | Стоимость хранения | `{{storage_fee}}` |
...
- Владелец процесса проверяет профиль «ответственное хранение авиагруза», стороны, authority matrix,
  коммерческие условия и evidence: storage receipt, AWB/HAWB, cargo manifest, condition photos, temperature log,
  access log, release instruction.
...
- Эскалация обязательна при событии: loss from storage, temperature excursion, unauthorized release или
  inventory mismatch. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «хранение, склад, ответственность, хранитель» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор ответственного хранения» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: storage receipt, AWB/HAWB, cargo manifest, condition photos, temperature log, access
  log, release instruction.
...
- Ограничение для ответа: хранение не подтверждает состояние груза, custody, special cargo режим или полномочия
  на выдачу. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей договорной
  нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор ответственного хранения» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «ответственное хранение авиагруза», liability cap, waiver и нестандартные приложения |
| Storage custody owner | подтверждает operational facts: storage receipt, AWB/HAWB, cargo manifest, condition photos, temperature log, access log, release instruction |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор ответственного хранения» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «ответственное хранение авиагруза»: storage
  receipt, AWB/HAWB, cargo manifest, condition photos, temperature log, access log, release instruction.
- Ключевой риск сценария: хранение не подтверждает состояние груза, custody, special cargo режим или полномочия
  на выдачу.
...
- ответ решает именно сценарий «ответственное хранение авиагруза» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: storage receipt, AWB/HAWB, cargo manifest, condition photos,
  temperature log, access log, release instruction
- учтен профильный риск: хранение не подтверждает состояние груза, custody, special cargo режим или полномочия
  на выдачу
...
- [ ] запрос сопоставлен с документом «Договор ответственного хранения» и профилем «ответственное хранение
  авиагруза»
- [ ] собраны профильные evidence: storage receipt, AWB/HAWB, cargo manifest, condition photos, temperature log,
  access log, release instruction
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: хранение не подтверждает состояние груза, custody, special cargo режим или полномочия
  на выдачу
...
Внеплановый пересмотр нужен после события: loss from storage, temperature excursion, unauthorized release или
inventory mismatch; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «ответственное хранение авиагруза» и
...
> Применять «Договор ответственного хранения» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
