...
Договор факторинга используется для сценария: финансируется портфель требований по freight, terminal handling,
storage или экспедиторским счетам. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется по
...
реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников, сверка оплат, dispute reserve.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «факторинг air cargo дебиторки» и есть операционная связка
...
- **Факторинг air cargo дебиторки** — профиль документа, который определяет обязательные приложения и evidence.
- **Aviation evidence по сделке** — реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников,
  сверка оплат, dispute reserve; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — в факторинг попадает disputed invoice без резерва, notice debtor или проверки
  права уступки; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — возражение должника, chargeback, страховой регресс или просрочка оплаты по группе
  AWB; после него проект возвращается к Legal, operations и finance/insurance.
...
- Владелец процесса проверяет профиль «факторинг air cargo дебиторки», стороны, authority matrix, коммерческие
  условия и evidence: реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников, сверка оплат,
  dispute reserve.
...
- Эскалация обязательна при событии: возражение должника, chargeback, страховой регресс или просрочка оплаты по
  группе AWB. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «факторинг, цессия, финансирование, дебиторка» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор факторинга» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников, сверка
  оплат, dispute reserve.
...
- Ограничение для ответа: в факторинг попадает disputed invoice без резерва, notice debtor или проверки права
  уступки. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей договорной
  нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор факторинга» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «факторинг air cargo дебиторки», liability cap, waiver и нестандартные приложения |
| Factoring portfolio owner | подтверждает operational facts: реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников, сверка оплат, dispute reserve |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор факторинга» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «факторинг air cargo дебиторки»: реестр
  уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников, сверка оплат, dispute reserve.
- Ключевой риск сценария: в факторинг попадает disputed invoice без резерва, notice debtor или проверки права
  уступки.
...
- ответ решает именно сценарий «факторинг air cargo дебиторки» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления
  должников, сверка оплат, dispute reserve
- учтен профильный риск: в факторинг попадает disputed invoice без резерва, notice debtor или проверки права
  уступки
...
- [ ] запрос сопоставлен с документом «Договор факторинга» и профилем «факторинг air cargo дебиторки»
- [ ] собраны профильные evidence: реестр уступаемых счетов, AWB/MAWB/HAWB, акты услуг, уведомления должников,
  сверка оплат, dispute reserve
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: в факторинг попадает disputed invoice без резерва, notice debtor или проверки права
  уступки
...
Внеплановый пересмотр нужен после события: возражение должника, chargeback, страховой регресс или просрочка
оплаты по группе AWB; также после изменения carrier/GHA rules, airport security требований, customs/export
control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «факторинг air cargo дебиторки» и
...
> Применять «Договор факторинга» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
