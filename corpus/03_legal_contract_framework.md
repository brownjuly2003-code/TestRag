...
Рамочный договор (framework agreement) используется для сценария: рамочный договор задает правила для
повторяющихся booking, SLA, тарифов и приложений. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
master agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix, terminal tariff, change log.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «рамочные условия авиагрузовых заявок» и есть операционная
...
- **Рамочные условия авиагрузовых заявок** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — master agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix,
  terminal tariff, change log; комплект подтверждает предмет, цену, сроки, приемку и пределы ответственности.
- **Операционное ограничение** — рамка подменяет конкретный order form и не фиксирует аэропорты, cutoff,
  chargeable weight или special cargo; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — конфликт master terms и приложения, изменение тарифа, лимита ответственности или
  маршрута; после него проект возвращается к Legal, operations и finance/insurance.
...
| `termination_notice` | Уведомление о расторжении | `{{termination_notice}}` |
...
- Уведомление о расторжении: {{termination_notice}}
...
- Владелец процесса проверяет профиль «рамочные условия авиагрузовых заявок», стороны, authority matrix,
  коммерческие условия и evidence: master agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix,
  terminal tariff, change log.
...
- Эскалация обязательна при событии: конфликт master terms и приложения, изменение тарифа, лимита
  ответственности или маршрута. Решение фиксируется в версии договора, приложении, протоколе разногласий или
  change log.
...
- Предметная проверка: признаки «рамочный договор, framework, спецификация, заказ» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Рамочный договор (framework agreement)» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: master agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix, terminal
  tariff, change log.
...
- Ограничение для ответа: рамка подменяет конкретный order form и не фиксирует аэропорты, cutoff, chargeable
  weight или special cargo. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его
...
|---------------------|-------------------------------|
| Подходит ли «Рамочный договор (framework agreement)» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «рамочные условия авиагрузовых заявок», liability cap, waiver и нестандартные приложения |
| Framework contract owner | подтверждает operational facts: master agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix, terminal tariff, change log |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Рамочный договор (framework agreement)» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «рамочные условия авиагрузовых заявок»: master
  agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix, terminal tariff, change log.
- Ключевой риск сценария: рамка подменяет конкретный order form и не фиксирует аэропорты, cutoff, chargeable
  weight или special cargo.
...
- ответ решает именно сценарий «рамочные условия авиагрузовых заявок» и не смешивает его со смежным договором
  или претензией
- источники подтверждения названы предметно: master agreement, order form, rate sheet, booking, AWB/HAWB, SLA
  appendix, terminal tariff, change log
- учтен профильный риск: рамка подменяет конкретный order form и не фиксирует аэропорты, cutoff, chargeable
  weight или special cargo
...
- [ ] запрос сопоставлен с документом «Рамочный договор (framework agreement)» и профилем «рамочные условия
  авиагрузовых заявок»
- [ ] собраны профильные evidence: master agreement, order form, rate sheet, booking, AWB/HAWB, SLA appendix,
  terminal tariff, change log
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: рамка подменяет конкретный order form и не фиксирует аэропорты, cutoff, chargeable
  weight или special cargo
...
Внеплановый пересмотр нужен после события: конфликт master terms и приложения, изменение тарифа, лимита
ответственности или маршрута; также после изменения carrier/GHA rules, airport security требований,
customs/export control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «рамочные условия авиагрузовых заявок» и
...
> Применять «Рамочный договор (framework agreement)» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
