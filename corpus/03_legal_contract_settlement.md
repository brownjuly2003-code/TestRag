...
Договор о зачете встречных требований используется для сценария: стороны закрывают претензию о freight,
damage/loss, storage, delay или customs costs. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
claim file, AWB/MAWB/HAWB, loss calculation, payment schedule, release wording, insurer consent, authority
check. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «мировое соглашение по air cargo спору» и есть операционная
...
- **Мировое соглашение по air cargo спору** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — claim file, AWB/MAWB/HAWB, loss calculation, payment schedule, release
  wording, insurer consent, authority check; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — settlement подписан без полномочий, insurer consent, tax review или сохранения
  прав по non-settled claims; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — missed installment, partial release dispute, recourse claim или confidentiality
  breach; после него проект возвращается к Legal, operations и finance/insurance.
...
| `freight_or_handling_fee` | Freight/handling fee | `{{freight_or_handling_fee}}` |
| `dispute_status` | Статус претензии по грузу | `{{dispute_status}}` |
...
2.1. Предмет: финансовое обязательство из авиагрузовой операции, счета за freight/handling/storage, страховой
     выплаты или штрафа за SLA.
...
- Freight/handling fee: {{freight_or_handling_fee}}
- Статус претензии по грузу: {{dispute_status}}
...
3.2. Исполнение подтверждается: AWB/MAWB/HAWB, invoice, акт услуг, терминальный акт, отчет экспедитора, сверка
     расчетов и претензионная переписка.
...
- Владелец процесса проверяет профиль «мировое соглашение по air cargo спору», стороны, authority matrix,
  коммерческие условия и evidence: claim file, AWB/MAWB/HAWB, loss calculation, payment schedule, release
  wording, insurer consent, authority check.
...
- Эскалация обязательна при событии: missed installment, partial release dispute, recourse claim или
  confidentiality breach. Решение фиксируется в версии договора, приложении, протоколе разногласий или change
  log.
...
- Сценарий: финансы оформляют уступку или зачет задолженности по нескольким AWB и счетам за handling.
- Проверить: основание требования, AWB/invoice, chargeable weight, тариф, статус претензий, зачеты и лимиты
  ответственности.
- Подтвердить источниками: AWB/MAWB/HAWB, invoice, акт услуг, терминальный акт, отчет экспедитора, сверка
  расчетов и претензионная переписка.
...
- Предметная проверка: признаки «зачет, встречные требования, взаимозачет» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор о зачете встречных требований» только при совпадении запроса с авиагрузовым предметом документа.
- Перед ответом проверить: основание требования, AWB/invoice, chargeable weight, тариф, статус претензий, зачеты
  и лимиты ответственности; полномочия подписантов, маршрут согласования, версию приложений и aviation evidence.
...
- Профильные источники: claim file, AWB/MAWB/HAWB, loss calculation, payment schedule, release wording, insurer
  consent, authority check.
...
- Ограничение для ответа: settlement подписан без полномочий, insurer consent, tax review или сохранения прав по
  non-settled claims. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор о зачете встречных требований» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «мировое соглашение по air cargo спору», liability cap, waiver и нестандартные приложения |
| Dispute settlement owner | подтверждает operational facts: claim file, AWB/MAWB/HAWB, loss calculation, payment schedule, release wording, insurer consent, authority check |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор о зачете встречных требований» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «мировое соглашение по air cargo спору»: claim
  file, AWB/MAWB/HAWB, loss calculation, payment schedule, release wording, insurer consent, authority check.
- Ключевой риск сценария: settlement подписан без полномочий, insurer consent, tax review или сохранения прав по
  non-settled claims.
...
- ответ решает именно сценарий «мировое соглашение по air cargo спору» и не смешивает его со смежным договором
  или претензией
- источники подтверждения названы предметно: claim file, AWB/MAWB/HAWB, loss calculation, payment schedule,
  release wording, insurer consent, authority check
- учтен профильный риск: settlement подписан без полномочий, insurer consent, tax review или сохранения прав по
  non-settled claims
...
- [ ] запрос сопоставлен с документом «Договор о зачете встречных требований» и профилем «мировое соглашение по
  air cargo спору»
- [ ] собраны профильные evidence: claim file, AWB/MAWB/HAWB, loss calculation, payment schedule, release
  wording, insurer consent, authority check
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: settlement подписан без полномочий, insurer consent, tax review или сохранения прав
  по non-settled claims
...
Внеплановый пересмотр нужен после события: missed installment, partial release dispute, recourse claim или
confidentiality breach; также после изменения carrier/GHA rules, airport security требований, customs/export
control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «мировое соглашение по air cargo спору» и
...
> Применять «Договор о зачете встречных требований» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
