...
Лицензионный договор на ПО используется для сценария: лицензируется ПО для AWB, booking, cargo manifest,
warehouse status или claims records. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется
...
license order, user list, data categories, API/EDI specs, SLA, audit log, support tickets, security review.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «лицензия cargo operations software» и есть операционная
...
- **Лицензия cargo operations software** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — license order, user list, data categories, API/EDI specs, SLA, audit log,
  support tickets, security review; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — лицензия не покрывает обработку AWB/customs data, API integration, backup или
  прекращение доступа; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — license overuse, data export failure, outage, audit dispute или vendor lock-in;
...
| `user_count` | Количество пользователей | `{{user_count}}` |
...
| `air_cargo_scope` | Air cargo scope | `{{air_cargo_scope}}` |
| `sla_metrics_air` | SLA для booking/handling | `{{sla_metrics_air}}` |
...
2.1. Предмет: оказание услуг для air cargo operations: booking support, terminal handling, AWB/cargo data,
     IT-интеграции, consulting или сопровождение грузовой емкости.
...
- Air cargo scope: {{air_cargo_scope}}
- SLA для booking/handling: {{sla_metrics_air}}
...
3.2. Исполнение подтверждается: ТЗ, SLA report, tickets, WMS/TMS logs, акты услуг, AWB/cargo data extracts,
     отчеты терминала и протоколы инцидентов.
...
- Владелец процесса проверяет профиль «лицензия cargo operations software», стороны, authority matrix,
  коммерческие условия и evidence: license order, user list, data categories, API/EDI specs, SLA, audit log,
  support tickets, security review.
...
- Эскалация обязательна при событии: license overuse, data export failure, outage, audit dispute или vendor
  lock-in. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Сценарий: исполнитель поддерживает услугу, от которой зависит выпуск HAWB и сдача груза в терминал до cutoff time.
- Проверить: объем услуги, cargo data, SLA, влияние на booking/AWB/cutoff, доступы, отчетность, акт и
  конфиденциальность.
- Подтвердить источниками: ТЗ, SLA report, tickets, WMS/TMS logs, акты услуг, AWB/cargo data extracts, отчеты
  терминала и протоколы инцидентов.
...
- Предметная проверка: признаки «лицензия, ПО, авторское право, сопровождение» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Лицензионный договор на ПО» только при совпадении запроса с авиагрузовым предметом документа.
- Перед ответом проверить: объем услуги, cargo data, SLA, влияние на booking/AWB/cutoff, доступы, отчетность,
  акт и конфиденциальность; полномочия подписантов, маршрут согласования, версию приложений и aviation evidence.
...
- Профильные источники: license order, user list, data categories, API/EDI specs, SLA, audit log, support
  tickets, security review.
...
- Ограничение для ответа: лицензия не покрывает обработку AWB/customs data, API integration, backup или
  прекращение доступа. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Лицензионный договор на ПО» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «лицензия cargo operations software», liability cap, waiver и нестандартные приложения |
| Software license owner | подтверждает operational facts: license order, user list, data categories, API/EDI specs, SLA, audit log, support tickets, security review |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Лицензионный договор на ПО» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «лицензия cargo operations software»: license
  order, user list, data categories, API/EDI specs, SLA, audit log, support tickets, security review.
- Ключевой риск сценария: лицензия не покрывает обработку AWB/customs data, API integration, backup или
  прекращение доступа.
...
- ответ решает именно сценарий «лицензия cargo operations software» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: license order, user list, data categories, API/EDI specs, SLA,
  audit log, support tickets, security review
- учтен профильный риск: лицензия не покрывает обработку AWB/customs data, API integration, backup или
  прекращение доступа
...
- [ ] запрос сопоставлен с документом «Лицензионный договор на ПО» и профилем «лицензия cargo operations
  software»
- [ ] собраны профильные evidence: license order, user list, data categories, API/EDI specs, SLA, audit log,
  support tickets, security review
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: лицензия не покрывает обработку AWB/customs data, API integration, backup или
  прекращение доступа
...
Внеплановый пересмотр нужен после события: license overuse, data export failure, outage, audit dispute или
vendor lock-in; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «лицензия cargo operations software» и
...
> Применять «Лицензионный договор на ПО» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
