...
Договор лизинга транспортных средств используется для сценария: в лизинг передается техника для ramp, warehouse,
cold chain или airport cargo operations. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
leasing schedule, equipment acceptance act, maintenance log, airport access permit, insurance policy, defect
report. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «лизинг GSE или спецтранспорта терминала» и есть
...
- **Лизинг gse или спецтранспорта терминала** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — leasing schedule, equipment acceptance act, maintenance log, airport access
  permit, insurance policy, defect report; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — техника используется без airport permit, maintenance SLA, страховки или правил
  dangerous goods/cold chain; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — простой техники, отказ допуска на airside, повреждение ULD или cold chain breach;
...
- Владелец процесса проверяет профиль «лизинг GSE или спецтранспорта терминала», стороны, authority matrix,
  коммерческие условия и evidence: leasing schedule, equipment acceptance act, maintenance log, airport access
  permit, insurance policy, defect report.
...
- Эскалация обязательна при событии: простой техники, отказ допуска на airside, повреждение ULD или cold chain
  breach. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «лизинг, транспорт, финансовая аренда, выкуп» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор лизинга транспортных средств» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: leasing schedule, equipment acceptance act, maintenance log, airport access permit,
  insurance policy, defect report.
...
- Ограничение для ответа: техника используется без airport permit, maintenance SLA, страховки или правил
  dangerous goods/cold chain. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его
...
|---------------------|-------------------------------|
| Подходит ли «Договор лизинга транспортных средств» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «лизинг GSE или спецтранспорта терминала», liability cap, waiver и нестандартные приложения |
| GSE fleet owner | подтверждает operational facts: leasing schedule, equipment acceptance act, maintenance log, airport access permit, insurance policy, defect report |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор лизинга транспортных средств» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «лизинг GSE или спецтранспорта терминала»:
  leasing schedule, equipment acceptance act, maintenance log, airport access permit, insurance policy, defect
  report.
- Ключевой риск сценария: техника используется без airport permit, maintenance SLA, страховки или правил
...
- ответ решает именно сценарий «лизинг GSE или спецтранспорта терминала» и не смешивает его со смежным договором
  или претензией
- источники подтверждения названы предметно: leasing schedule, equipment acceptance act, maintenance log,
  airport access permit, insurance policy, defect report
- учтен профильный риск: техника используется без airport permit, maintenance SLA, страховки или правил
  dangerous goods/cold chain
...
- [ ] запрос сопоставлен с документом «Договор лизинга транспортных средств» и профилем «лизинг GSE или
  спецтранспорта терминала»
- [ ] собраны профильные evidence: leasing schedule, equipment acceptance act, maintenance log, airport access
  permit, insurance policy, defect report
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: техника используется без airport permit, maintenance SLA, страховки или правил
  dangerous goods/cold chain
...
Внеплановый пересмотр нужен после события: простой техники, отказ допуска на airside, повреждение ULD или cold
chain breach; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «лизинг GSE или спецтранспорта терминала»
...
> Применять «Договор лизинга транспортных средств» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
