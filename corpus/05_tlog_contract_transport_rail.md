...
title: Договор фидерного плеча авиагруза до/из аэропорта
...
- **Рабочий экземпляр «Договор фидерного плеча авиагруза до/из аэропорта»** — версия документа с датой, владельцем процесса, статусом согласования и приложениями, по которой можно принимать внутреннее решение.
- **Основание применения «Договор фидерного плеча авиагруза до/из аэропорта»** — заявка, приказ, договор, акт, обращение или иной первичный документ, без которого ассистент не должен формировать окончательный вывод.
...
| `feeder_unit_count` | Количество фидерных единиц | `{{feeder_unit_count}}` |
...
- Количество фидерных единиц: {{feeder_unit_count}}
...
- Владелец процесса проверяет, что «Договор фидерного плеча авиагруза до/из аэропорта» применяется к
  подтвержденной air cargo операции, а не к общей перевозке без booking, AWB/MAWB/HAWB и terminal record.
- Рабочий комплект: rail feeder link, terminal handover, AWB connection, cargo seal continuity и transfer
  акт. CMR/ТТН, GPS и данные водителя используются только для наземного подвоза или вывоза, если этот
  участок связан с airport pair и рейсом.
- Срок реакции: контроль выполняется до подтверждения стыковки с авиарейсом; missed cutoff, failed
...
- Основания для HOLD/STOP: железнодорожная накладная подменяет AWB или не подтверждена передача в терминал.
  Неподтвержденные даты, суммы, персональные данные, пломбы, вес, temperature mode и реквизиты не
  подставляются.
- Маршрут эскалации: rail forwarder, air cargo dispatcher, terminal, customs и legal; спорный RAG-ответ,
...
- Предметная проверка: признаки «фидерное плечо авиагруза, airport terminal, AWB, booking» сверяются с исходными документами и не используются как общий ярлык.

## Air cargo профиль документа

- Для «Договор фидерного плеча авиагруза до/из аэропорта» air cargo профиль строится вокруг подтвержденного
...
- Предметная опора документа: rail feeder link, terminal handover, AWB connection, cargo seal continuity и
  transfer акт. Эти факты должны быть видны в TMS/WMS/terminal log, акте, фото, переписке carrier/GSA или
  связанном документе.
...
- Special cargo требует ручной проверки для dangerous goods, lithium batteries, pharma/cold chain,
  perishables, oversized и valuable cargo; HOLD наступает, если железнодорожная накладная подменяет AWB или
  не подтверждена передача в терминал.
- Эскалация идет через rail forwarder, air cargo dispatcher, terminal, customs и legal, чтобы документ не
  превращался в общий транспортный шаблон без авиационных доказательств.
## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор перевозки груза» по теме «Договор фидерного плеча авиагруза до/из аэропорта» только при фактическом совпадении запроса с предметом документа.
...
- Если фактов по «Договор фидерного плеча авиагруза до/из аэропорта» не хватает, вернуть уточняющие вопросы; даты, суммы, доступы и реквизиты оставлять неподтвержденными до проверки.
...
- ключевые признаки документа: фидерное плечо авиагруза, airport terminal, AWB, booking.
- нормативные ориентиры: ГК РФ, глава 40 (перевозка); Воздушный кодекс РФ и правила воздушных перевозок грузов.
...
| Какие факты нужны для «Договор фидерного плеча авиагруза до/из аэропорта»? | Перечислить недостающие даты, документы, участников, суммы/сроки и ответственных. |
| Можно ли применять автоматически? | Только после проверки ограничений по «Договор фидерного плеча авиагруза до/из аэропорта»; при риске или неполных фактах нужна ручная проверка. |
...
| Терминал / склад / наземное плечо | передает для «Договор фидерного плеча авиагруза до/из аэропорта» booking, AWB, terminal records, CMR/ТТН и GPS только как first/last mile evidence |
...
- ключевые признаки (фидерное плечо авиагруза, airport terminal, AWB, booking) могут быть неверно применены без документа-основания.
...
- ответ решает именно сценарий «Договор фидерного плеча авиагруза до/из аэропорта» и не подменяет его общим правилом категории.
...
- ключевые признаки (фидерное плечо авиагруза, airport terminal, AWB, booking) использованы только вместе с подтвержденными документами.
...
- [ ] запрос сопоставлен с документом «Договор фидерного плеча авиагруза до/из аэропорта» и типом «договор перевозки груза»
...
- [ ] недостающие факты по «Договор фидерного плеча авиагруза до/из аэропорта» вынесены в уточняющие вопросы
...
> Применять «Договор фидерного плеча авиагруза до/из аэропорта» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
