---
doc_id: legal_contract_loan
title: Договор займа (между юрлицами)
...
Договор займа (между юрлицами) используется для сценария: финансируются carrier prepayment, terminal charges,
customs deposits или оборот по группе AWB. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет
...
loan agreement, cash flow forecast, AWB/invoice pool, payment calendar, covenant report, collateral file.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «заем под air cargo оборот» и есть операционная связка с
...
- **Заем под air cargo оборот** — профиль документа, который определяет обязательные приложения и evidence.
- **Aviation evidence по сделке** — loan agreement, cash flow forecast, AWB/invoice pool, payment calendar,
  covenant report, collateral file; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — заем не связан с подтвержденной выручкой, сроками оплаты или ограничениями
  целевого использования; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — breach of covenant, delayed receivables, customs deposit overrun или
  cross-default; после него проект возвращается к Legal, operations и finance/insurance.
...
| `lender_name` | Наименование заимодавца | `{{lender_name}}` |
| `borrower_name` | Заемщик | `{{borrower_name}}` |
| `loan_amount` | Сумма займа | `{{loan_amount}}` |
| `interest_rate` | Ставка процентов | `{{interest_rate}}` |
| `maturity_date` | Дата погашения | `{{maturity_date}}` |
| `repayment_schedule` | График погашения | `{{repayment_schedule}}` |
...
- Наименование заимодавца: {{lender_name}}
- Заемщик: {{borrower_name}}
- Сумма займа: {{loan_amount}}
- Ставка процентов: {{interest_rate}}
- Дата погашения: {{maturity_date}}
- График погашения: {{repayment_schedule}}
...
- Владелец процесса проверяет профиль «заем под air cargo оборот», стороны, authority matrix, коммерческие
  условия и evidence: loan agreement, cash flow forecast, AWB/invoice pool, payment calendar, covenant report,
  collateral file.
...
- Эскалация обязательна при событии: breach of covenant, delayed receivables, customs deposit overrun или
  cross-default. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «заем, займ, проценты, ссуда» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор займа (между юрлицами)» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: loan agreement, cash flow forecast, AWB/invoice pool, payment calendar, covenant report,
  collateral file.
...
- Ограничение для ответа: заем не связан с подтвержденной выручкой, сроками оплаты или ограничениями целевого
  использования. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор займа (между юрлицами)» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «заем под air cargo оборот», liability cap, waiver и нестандартные приложения |
| Treasury owner | подтверждает operational facts: loan agreement, cash flow forecast, AWB/invoice pool, payment calendar, covenant report, collateral file |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор займа (между юрлицами)» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «заем под air cargo оборот»: loan agreement,
  cash flow forecast, AWB/invoice pool, payment calendar, covenant report, collateral file.
- Ключевой риск сценария: заем не связан с подтвержденной выручкой, сроками оплаты или ограничениями целевого
  использования.
...
- ответ решает именно сценарий «заем под air cargo оборот» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: loan agreement, cash flow forecast, AWB/invoice pool, payment
  calendar, covenant report, collateral file
- учтен профильный риск: заем не связан с подтвержденной выручкой, сроками оплаты или ограничениями целевого
  использования
...
- [ ] запрос сопоставлен с документом «Договор займа (между юрлицами)» и профилем «заем под air cargo оборот»
- [ ] собраны профильные evidence: loan agreement, cash flow forecast, AWB/invoice pool, payment calendar,
  covenant report, collateral file
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: заем не связан с подтвержденной выручкой, сроками оплаты или ограничениями целевого
  использования
...
Внеплановый пересмотр нужен после события: breach of covenant, delayed receivables, customs deposit overrun или
cross-default; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «заем под air cargo оборот» и обновляет
...
> Применять «Договор займа (между юрлицами)» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
