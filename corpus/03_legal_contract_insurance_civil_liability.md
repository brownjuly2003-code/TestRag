...
Договор страхования гражданской ответственности перевозчика используется для сценария: покрываются liability
claims по handling, forwarding, storage и carrier-related событиям. Это рабочий шаблон Legal для авиагрузовой
сделки, где предмет проверяется по источникам, а не по названию документа.

До выпуска проекта юрист сверяет полномочия, приложения, лимиты ответственности, страхование и доказательства:
policy wording, insured operations list, AWB/HAWB, terminal act, claim notice, loss calculation, reserve
correspondence. AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как
условия или ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «страхование ответственности перевозчика/экспедитора/GHA» и
есть операционная связка с air cargo route, terminal handover или расчетами по перевозке.
...
- **Страхование ответственности перевозчика/экспедитора/gha** — профиль документа, который определяет
  обязательные приложения и evidence.
- **Aviation evidence по сделке** — policy wording, insured operations list, AWB/HAWB, terminal act, claim
  notice, loss calculation, reserve correspondence; комплект подтверждает предмет, цену, сроки, приемку и
  пределы ответственности.
- **Операционное ограничение** — полис не совпадает с фактической ролью стороны, лимитом ответственности или
  территорией аэропорта; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — third-party claim, insurer reservation of rights, deductible dispute или late
  notice; после него проект возвращается к Legal, operations и finance/insurance.
...
2.1. Предмет: страхование ответственности перевозчика, экспедитора или GHA за claims по handling, forwarding,
     storage, release и carrier-related событиям.
...
3.2. Исполнение подтверждается: policy wording, insured operations list, AWB/HAWB, terminal act, claim notice,
     loss calculation, reserve correspondence и insurer position.
...
- Владелец процесса проверяет профиль «страхование ответственности перевозчика/экспедитора/GHA», стороны,
  authority matrix, коммерческие условия и evidence: policy wording, insured operations list, AWB/HAWB, terminal
  act, claim notice, loss calculation, reserve correspondence.
...
- Эскалация обязательна при событии: third-party claim, insurer reservation of rights, deductible dispute или
  late notice. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Подтвердить источниками: policy wording, insured operations list, AWB/HAWB, terminal act, claim notice, loss
  calculation, reserve correspondence и insurer position.
...
- Предметная проверка: признаки «ОСГО, перевозчик, ответственность, страхование» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор страхования гражданской ответственности перевозчика» только при совпадении запроса с авиагрузовым предметом документа.
- Перед ответом проверить: insured role, covered operations, liability cap, deductible, notice period,
  territory, AWB/HAWB, terminal act, полномочия подписантов и insurer approval.
...
- Профильные источники: policy wording, insured operations list, AWB/HAWB, terminal act, claim notice, loss
  calculation, reserve correspondence.
...
- Ограничение для ответа: полис не совпадает с фактической ролью стороны, лимитом ответственности или
  территорией аэропорта. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей
  договорной нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор страхования гражданской ответственности перевозчика» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «страхование ответственности перевозчика/экспедитора/GHA», liability cap, waiver и нестандартные приложения |
| Liability insurance owner | подтверждает operational facts: policy wording, insured operations list, AWB/HAWB, terminal act, claim notice, loss calculation, reserve correspondence |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор страхования гражданской ответственности перевозчика» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «страхование ответственности
  перевозчика/экспедитора/GHA»: policy wording, insured operations list, AWB/HAWB, terminal act, claim notice,
  loss calculation, reserve correspondence.
- Ключевой риск сценария: полис не совпадает с фактической ролью стороны, лимитом ответственности или
  территорией аэропорта.
...
- ответ решает именно сценарий «страхование ответственности перевозчика/экспедитора/GHA» и не смешивает его со
  смежным договором или претензией
- источники подтверждения названы предметно: policy wording, insured operations list, AWB/HAWB, terminal act,
  claim notice, loss calculation, reserve correspondence
- учтен профильный риск: полис не совпадает с фактической ролью стороны, лимитом ответственности или территорией
  аэропорта
...
- [ ] запрос сопоставлен с документом «Договор страхования гражданской ответственности перевозчика» и профилем
  «страхование ответственности перевозчика/экспедитора/GHA»
- [ ] собраны профильные evidence: policy wording, insured operations list, AWB/HAWB, terminal act, claim
  notice, loss calculation, reserve correspondence
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: полис не совпадает с фактической ролью стороны, лимитом ответственности или
  территорией аэропорта
...
Внеплановый пересмотр нужен после события: third-party claim, insurer reservation of rights, deductible dispute
или late notice; также после изменения carrier/GHA rules, airport security требований, customs/export control
режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «страхование ответственности
перевозчика/экспедитора/GHA» и обновляет evidence list, сроки согласования, роли эскалации и ограничения
RAG-ответа.
...
> Применять «Договор страхования гражданской ответственности перевозчика» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
