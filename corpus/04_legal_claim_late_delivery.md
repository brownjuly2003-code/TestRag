...
title: Претензия о задержке рейса или offload авиагруза
...
# Претензия о задержке рейса или offload авиагруза

## Назначение

Претензия о задержке рейса или offload авиагруза используется для сценария: груз не прибыл в согласованный срок
из-за offload, missed cutoff, rebooking или late terminal release. Это претензия контрагенту для конкретного air
cargo dispute, а не общая форма без shipment evidence.

Позиция строится на доказательствах: booking, AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps,
customs status, SLA, loss calculation. AWB/MAWB/HAWB, airport pair, flight number, terminal/GHA records, customs
и dangerous goods учитываются только как проверяемые факты, ограничения или причины эскалации.

## Область применения

Документ применяется, когда карточка спора подтверждает профиль «задержка рейса, offload или missed cutoff»,
...
признания вины или долга. Для события «carrier escalation, customer notice или mitigation rebooking» Legal
согласует escalation route с operations, finance, insurer, broker или external counsel.

## Термины

- **Задержка рейса, offload или missed cutoff** — профиль претензии или письма, который определяет предмет
  доказывания и безопасную позицию.
- **Evidence bundle** — booking, AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps, customs
  status, SLA, loss calculation; комплект используется для chronology, liability, amount, mitigation и вручения.
- **Без признания** — ответ не подтверждает долг, вину, causation, размер ущерба, waiver of rights или отказ от
  liability cap.
- **Escalation trigger** — carrier escalation, customer notice или mitigation rebooking; событие требует решения
...
- Владелец спора сверяет профиль «задержка рейса, offload или missed cutoff», карточку спора, срок ответа,
  способ вручения и evidence: booking, AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps,
  customs status, SLA, loss calculation.
...
- Эскалация обязательна при событии: carrier escalation, customer notice или mitigation rebooking. До решения
...
- Сценарий: груз offloaded с планового рейса и доставлен на 48 часов позже без notice consignee.
...
- Подтвердить источниками: AWB/MAWB/HAWB, booking, flight number, cargo manifest, акт терминала или GHA, фото повреждения или расхождения, отметки screening/handling/customs, temperature log при special cargo, invoices/rate sheet, переписка с авиаперевозчиком, GHA, экспедитором, customs broker и страховщиком. Дополнительно: booking history, flight status, offload notice, rebooking record, terminal timestamps.
...
- Использовать документ как источник типа «претензия контрагенту» по теме «Претензия о задержке рейса или offload авиагруза» только при фактическом совпадении запроса с air cargo dispute.
...
- Основные источники: booking, AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps, customs
  status, SLA, loss calculation.
...
- Проверяемое ограничение: нельзя предъявлять delay damages без planned/actual timeline и исключения
  customs/weather/shipper delay.
...
| Какие факты нужны для «Претензия о задержке рейса или offload авиагруза»? | Перечислить недостающие AWB, booking, рейс, airport pair, timeline события, сумму, доказательства вручения и ответственных. |

## Ответственные роли

| Роль | Зона ответственности |
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «задержка рейса, offload или missed cutoff», settlement authority, waiver и спорные liability caps |
| Delay claims owner | ведет chronology, evidence bundle, срок ответа и коммуникацию по событию: carrier escalation, customer notice или mitigation rebooking |
| Air cargo operations / GHA contact | подтверждает для «Претензия о задержке рейса или offload авиагруза» movement, acceptance/offload/release, terminal act, handling records и operational root cause |
| Finance / insurer / broker contact | сверяет по «Претензия о задержке рейса или offload авиагруза» сумму требования, reserve, оплату, customs costs, insurance notice и право регресса |

## Риски и ограничения

- Нельзя направлять документ без evidence по профилю «задержка рейса, offload или missed cutoff»: booking,
  AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps, customs status, SLA, loss calculation.
- Ключевое ограничение: нельзя предъявлять delay damages без planned/actual timeline и исключения
  customs/weather/shipper delay.
...
- ответ относится к профилю «задержка рейса, offload или missed cutoff» и не подменяет его общей претензионной
  процедурой
- доказательства названы предметно: booking, AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps,
  customs status, SLA, loss calculation
- отдельно указано ограничение или gap: нельзя предъявлять delay damages без planned/actual timeline и
  исключения customs/weather/shipper delay
- срок ответа, способ вручения, escalation route и safe legal position отражены явно

## Контрольный список

- [ ] карточка спора сопоставлена с документом «Претензия о задержке рейса или offload авиагруза» и профилем
  «задержка рейса, offload или missed cutoff»
- [ ] собран evidence bundle: booking, AWB/MAWB/HAWB, flight status, offload notice, terminal timestamps,
  customs status, SLA, loss calculation
- [ ] проверены claim notice, limitation period, proof of delivery/service и authority signer
- [ ] оценено ограничение: нельзя предъявлять delay damages без planned/actual timeline и исключения
  customs/weather/shipper delay
...
Внеплановый пересмотр нужен после события: carrier escalation, customer notice или mitigation rebooking; также
...
При пересмотре Legal проверяет закрытые дела профиля «задержка рейса, offload или missed cutoff»: какие evidence
...
> Применять «Претензия о задержке рейса или offload авиагруза» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
