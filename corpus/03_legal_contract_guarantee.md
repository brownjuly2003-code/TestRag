...
Договор поручительства используется для сценария: поручитель или гарант покрывает оплату freight, storage,
penalties или возмещение. Это рабочий шаблон Legal для авиагрузовой сделки, где предмет проверяется по
...
guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice, debt reconciliation, liability cap.
AWB/MAWB/HAWB, airport, flight, terminal, customs и dangerous goods указываются только как условия или
ограничения.

## Область применения

Документ применяется, когда запрос связан с профилем «обеспечение air cargo обязательств» и есть операционная
...
- **Обеспечение air cargo обязательств** — профиль документа, который определяет обязательные приложения и
  evidence.
- **Aviation evidence по сделке** — guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice,
  debt reconciliation, liability cap; комплект подтверждает предмет, цену, сроки, приемку и пределы
  ответственности.
- **Операционное ограничение** — гарантия не привязана к обеспечиваемому обязательству, сроку требования или
  комплекту evidence; условие выносится в приложение, SLA или протокол разногласий.
- **Эскалационное событие** — default debtor, demand under guarantee, partial payment или dispute over secured
  charges; после него проект возвращается к Legal, operations и finance/insurance.
...
- Владелец процесса проверяет профиль «обеспечение air cargo обязательств», стороны, authority matrix,
  коммерческие условия и evidence: guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice,
  debt reconciliation, liability cap.
...
- Эскалация обязательна при событии: default debtor, demand under guarantee, partial payment или dispute over
  secured charges. Решение фиксируется в версии договора, приложении, протоколе разногласий или change log.
...
- Предметная проверка: признаки «поручительство, гарант, обеспечение» используются только вместе с подтвержденными AWB/booking, приложениями и операционными документами.

## Правила ответа RAG-ассистента

- Использовать документ как источник типа «договор» по теме «Договор поручительства» только при совпадении запроса с авиагрузовым предметом документа.
...
- Профильные источники: guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice, debt
  reconciliation, liability cap.
...
- Ограничение для ответа: гарантия не привязана к обеспечиваемому обязательству, сроку требования или комплекту
  evidence. Если источник не подтверждает его, RAG-ответ указывает пробел и не подменяет его общей договорной
  нормой.
...
|---------------------|-------------------------------|
| Подходит ли «Договор поручительства» для авиагрузовой сделки? | Сопоставить предмет, стороны, AWB/booking, аэропорты, SLA, special cargo и обязательные приложения; назвать недостающие факты и риски. |
...
|------|----------------------|
| Руководитель юридического отдела | утверждает позицию по профилю «обеспечение air cargo обязательств», liability cap, waiver и нестандартные приложения |
| Security package owner | подтверждает operational facts: guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice, debt reconciliation, liability cap |
| Finance / insurance owner | сверяет тарифы, оплату, reserves, страховое покрытие, deductible и право регресса |
| Compliance / customs contact | проверяет для «Договор поручительства» sanctions/export control, dangerous goods, airport security и ограничения передачи данных |

## Риски и ограничения

- Нельзя готовить договор или RAG-ответ без evidence по профилю «обеспечение air cargo обязательств»: guarantee
  text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice, debt reconciliation, liability cap.
- Ключевой риск сценария: гарантия не привязана к обеспечиваемому обязательству, сроку требования или комплекту
  evidence.
...
- ответ решает именно сценарий «обеспечение air cargo обязательств» и не смешивает его со смежным договором или
  претензией
- источники подтверждения названы предметно: guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand
  notice, debt reconciliation, liability cap
- учтен профильный риск: гарантия не привязана к обеспечиваемому обязательству, сроку требования или комплекту
  evidence
...
- [ ] запрос сопоставлен с документом «Договор поручительства» и профилем «обеспечение air cargo обязательств»
- [ ] собраны профильные evidence: guarantee text, underlying contract, AWB/MAWB/HAWB, invoice, demand notice,
  debt reconciliation, liability cap
- [ ] проверены полномочия подписанта, версия приложений, liability cap, insurance и payment terms
- [ ] оценено ограничение: гарантия не привязана к обеспечиваемому обязательству, сроку требования или комплекту
  evidence
...
Внеплановый пересмотр нужен после события: default debtor, demand under guarantee, partial payment или dispute
over secured charges; также после изменения carrier/GHA rules, airport security требований, customs/export
control режима, страхового покрытия или судебной практики.

При пересмотре Legal выборочно сверяет 3-5 завершенных кейсов профиля «обеспечение air cargo обязательств» и
...
> Применять «Договор поручительства» после подтверждения владельцем процесса; спорные и нетиповые кейсы требуют ручной проверки.
